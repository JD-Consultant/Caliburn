...
def test_known_correction_supersedes_prior_evidence(gate_results):
    ...
    prior_id = prior_evidence_id_map(
        inputs, trial_id=result.ids.trial_id
...
def test_unknown_correction_does_not_supersede(gate_results):
    ...
    for key in (
        ...
        prior_id = prior_evidence_id_map(
            gate_results["TI-10-unknown-correction-target"][0],
            trial_id=result.ids.trial_id,
        )[key]
        status = {e.evidence_id: e.status for e in result.state_after.evidence}
...
def test_zero_evidence_case_commits_a_typed_receipt(gate_results):
    inputs, evaluation, result = gate_results["TI-11-zero-evidence"]
    assert result.output.literal_observations == ()
    assert result.receipt.accepted_evidence_ids == ()
    assert result.state_after_hash != result.state_before_hash
    reasons = {item.value for item in result.output.turn_insufficiency_codes}
...
def test_injection_case_anchors_the_actual_occurrence(gate_results):
    ...
    claims = " ".join(o.claim for o in result.output.literal_observations)
...
def test_prior_evidence_becomes_correction_candidates_in_context(gate_results):
    ...
    inputs = gate_results["TI-09-known-correction"][0]
    assert prior_evidence_id_map(
        inputs, trial_id=result.ids.trial_id
    )["prior-inventory-report-frequency"] in candidate_ids
...
def test_broken_reference_output_fails_the_gate(suite):
    ...
    broken_output = reference.output.model_copy(update={"literal_observations": ()})
