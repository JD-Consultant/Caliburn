...
fallback 觸發記 trace、Retry-After 尊重、sampling 參數過濾(2026-07-13 換模探針)。"""
import asyncio
...
from app.adapters.llm_openrouter import (
    _record_usage, _retry_after_seconds, openrouter_extra_body, sampling_params,
)
...
def test_sampling_params_filters_temperature_for_reasoning_models():
    """GPT-5/o 系不支援 temperature(supported_parameters 無此參數);
    配 require_parameters 送了=整路 404——必須過濾(探針:specs/2026-07-13)。"""
    assert sampling_params("openai/gpt-5.4-mini", 0) == {}
    assert sampling_params("openai/o4-mini", 0.4) == {}
    assert sampling_params("openai/gpt-4.1-mini", 0.4) == {"temperature": 0.4}
    assert sampling_params("deepseek/deepseek-v4-flash", 0.2) == {"temperature": 0.2}
...
def test_chat_with_tools_request_body(monkeypatch):
    """請求體斷言:extra_body(provider/require_parameters);**不送** parallel_tool_calls
    (OpenAI 預設即並行;顯式送+require_parameters 會 404——探針證實 4.1-mini 中招)。"""
    captured: dict = {}
    ...
    assert captured["temperature"] == 0.4            # 4.1-mini 支援 → 照送


def test_chat_with_tools_omits_temperature_for_gpt5(monkeypatch):
    """interview 若換 GPT-5 系:temperature 必須被濾掉,否則 require_parameters 404。"""
    captured: dict = {}
    ...
    fake = SimpleNamespace(chat=SimpleNamespace(completions=_FakeCompletions()))
    monkeypatch.setattr(llm_mod, "_async_client", lambda: fake)
    monkeypatch.setattr(settings, "model_interview", "openai/gpt-5.4-mini")

    async def dispatch(name, args):
        return {}

    asyncio.run(llm_mod.OpenRouterLlm().chat_with_tools(
        role="interview", messages=[{"role": "user", "content": "hi"}],
        tools=[{"type": "function", "function": {"name": "t", "parameters": {}}}],
        dispatch=dispatch))
