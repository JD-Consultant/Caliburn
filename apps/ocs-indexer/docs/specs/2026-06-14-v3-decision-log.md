...
### E4. reranker：加 bge-reranker-v2-m3 → **反轉：先擱置、量測再加**（2026-06-15）
- **原決定**：/search hybrid 後加 cross-encoder rerank。理由：2026 主流預設 hybrid+rerank（+26/28% recall/precision）；BGE 家族、self-host 零摩擦。
- **反轉（最終）**：**Phase 3 擱置 reranker**，先用真實查詢量測再決定。
- **為什麼反轉**：(1) 全量 ocs_v3（9,453 點）上 /search 召回已經很準（profile/task top-1 都對）；(2) 兩個用途都 human-in-the-loop（profile 給人選 1主N副、task K/S 在 Step 5 可編輯），排序微偏會被接住，precision@1 產品價值低；(3) reranker 紅利在「大/雜/query 模糊」語料才明顯，這裡是小而聚焦 + BGE-M3 夠強；(4) cross-encoder 重排 CPU 多 1-3 秒/query 傷互動體驗（**註：後來發現本機有 RTX 4060，GPU 下延遲不是阻礙——但「召回已夠好 + human-in-loop」仍成立，故維持擱置**）；(5) hybrid 的 sparse 已涵蓋 tool 詞精確命中；(6) YAGNI——還沒量到精準度問題。
- **何時回頭加**：jobintel-ai 接上後真實 query 出現 recall@k 漏 / task K/S 常明顯錯。且先試更便宜的：調 RRF prefetch_limit、dense/sparse 權重、query 前處理。
...
- **決定**：`/search`、`/task-pool` 每筆 task 多回 Qdrant **point id**（uuid）。jobintel-ai 記住即可日後 by-id retrieve（D3 沒改任務走捷徑）。
- **後續（Phase 2.5，2026-06-15）**：retrieve 端點已加 → **`POST /tasks/by-id {ids:[…]}`** 回完整 task payload。對流程的「by-id」機制(Step 4 output_pairs + Step 5 沒改 K/S)。同批也補了 `/pairs` 的 prerequisites/supplements(Step 7a/7b)。
