...
- [`specs/2026-07-31-anthropic-strict-schema-grammar-limit-research.md`](specs/2026-07-31-anthropic-strict-schema-grammar-limit-research.md) — **strict schema grammar 上限查證與修法分析**：Anthropic 官方**未公開**任何 union／深度／grammar 體積上限（strict tool use 與 structured outputs 共用編譯管線，故錯誤訊息誤稱 tool）；本產品 schema 展開後 54 property／17 nullable union／9 層巢狀（含三層巢狀陣列），遠低於 OpenAI 公布上限仍被拒。診斷指出綁定條件可能是巢狀陣列的乘性分支而非 optional 數量，故建議先做 ≈US$0.04 的零成本 grammar 探針量出門檻，再決定「中性值 wire schema ＋ mapper」或非 strict，並據實測開 ADR。
