...
- [`plans/2026-07-29-job-analysis-postgresql-persistence-plan.md`](plans/2026-07-29-job-analysis-postgresql-persistence-plan.md) — **Active greenfield durable vertical 計畫**：先擴充 Current JD Task 契約，再依序完成純 persistence ports、0012 四表 migration、PostgreSQL adapter、員工 direct edit、兩段式 AI turn、Proposal 決策與 reload smoke；不搬舊資料、不接 Web、不預建 O/P/K/S/A。
...
- [`adr/`](adr/) — Architecture Decision Records(決策的「為什麼」+ 取捨;**0001–0043**)；**現行 AI 層方向以 [ADR 0040](adr/0040-professional-consultant-engine-and-r1-validation-contract.md) 為基礎，第一版 Context 邊界由 [ADR 0041](adr/0041-r1-p0-closure-first-version-context-and-holdout.md) 補充，時程風險接受與 A6 第一版預設以 [ADR 0042](adr/0042-r1-screening-stop-and-a6-first-version-default.md) 為準；本機 Current State persistence 與分層編輯 authority 見 [ADR 0043](adr/0043-job-analysis-local-current-state-persistence-and-authoring-authority.md)**。provider 主線為 ADR 0035。0038、0039 已被取代，勿據以開新工。
