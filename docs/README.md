...
- [`specs/2026-07-20-interview-vnext-professional-job-analysis-and-short-answer-architecture-research.md`](specs/2026-07-20-interview-vnext-professional-job-analysis-and-short-answer-architecture-research.md) — **R5 前置、尚待 owner 裁決的職務分析與員工共編研究（Revision 4）**：產品是「員工與 AI 顧問對話 + 同步 JD canvas」，員工直接編輯立即成 draft truth，AI 只能建立可接受／修改／拒絕的 proposal；第一版不做 SaaS、公司 catalog 或多人協作。R5 前需先凍結 QuestionFrame + AnswerBinding + literal Evidence／ContextualAssertion 分型與 frequency/current 規則；editor 選型、K/S prompt、retrieval 與公版匯出不阻塞 R5。並定義 JobStateDigest、document-local custom K/S、canonical revisions/commands/proposals 與最小產品垂直切片。未回寫 mother plan 前不得直接實作。
