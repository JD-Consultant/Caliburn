...
- [`specs/2026-07-20-interview-vnext-professional-job-analysis-and-short-answer-architecture-research.md`](specs/2026-07-20-interview-vnext-professional-job-analysis-and-short-answer-architecture-research.md) — **R5 前置、尚待 owner 裁決的職務分析研究（Revision 3）**：目標收斂為「一個人當下工作→這一份專屬 JD」，不做公司 capability catalog、跨 JD K/S 共用或人才評分；提出 QuestionFrame + AnswerBinding + composite provenance 解決短回答，並定義公版 usable/partial/no-match、document-local custom K/S、task linkage、canonical JobModel authoring core 與可替換 editor/index/API adapters。既有元件不是限制；未回寫 mother plan 前不得直接實作。
