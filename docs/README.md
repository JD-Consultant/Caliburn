...
- [`specs/2026-07-30-job-analysis-local-web-authoring-contract-and-commit-seam-research.md`](specs/2026-07-30-job-analysis-local-web-authoring-contract-and-commit-seam-research.md) — **Proposed Local Web 第一切片研究**：多份可保存但一次只開一份；先收斂人／AI 共用 Current JD commit seam，再以 JSON Schema 生成 Python／TypeScript workspace DTO，greenfield 接文件庫與 Task 編輯，不接舊 user/profile/OCS 資料流。
...
- [`adr/0045-job-analysis-local-web-contract-and-shared-current-jd-commit.md`](adr/0045-job-analysis-local-web-contract-and-shared-current-jd-commit.md) — **Local Web 與共用 Current JD 寫入提案（Proposed，2026-07-30）**：新 `/workspace` 與 `/job-analysis/documents` seam、JSON Schema→Pydantic/TypeScript contract、人／AI 最終寫 JD 共用完整驗證與原子 commit；第一切片暫不含 AI UI。
