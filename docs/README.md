...
- [`specs/2026-07-30-professional-consultant-minimal-complete-loop-research.md`](specs/2026-07-30-professional-consultant-minimal-complete-loop-research.md) — **待 owner 書面審閱的專業顧問最小完整迴圈設計**：沿用 A6 強模型＋light schema＋one-stage，以固定持久開場、全訊號理解、單一最高資訊價值下一題、Proposal 與訪談並行及三個薄 API 接通可關閉重開的 AI Task 訪談；暫不新增 Role/Coverage/Story、Graph、多 Agent、streaming 或 O/P/K/S/A。
