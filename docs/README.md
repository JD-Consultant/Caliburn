...
- [`specs/2026-07-31-anthropic-strict-schema-grammar-limit-research.md`](specs/2026-07-31-anthropic-strict-schema-grammar-limit-research.md) — **strict schema grammar 上限查證與修法分析**：Anthropic 官方明列 optional 24／union parameters 16（strict tool use 與 structured outputs 共用編譯管線，故錯誤訊息誤稱 tool）。本產品 schema 送出時 13 union、**`$ref` 展開後 17 → 超出 16**；optional 17 在上限內不是主因。編譯器計數展開後形狀，`$defs` 不省額度，`TaskFields` 內聯兩次是主要來源。採「零 `anyOf` 的中性值 wire schema ＋ 純 mapper」，以最多兩次丟棄式探針驗證（Probe U 消 union；僅在 U 失敗才做 Probe F 扁平化最深 split 路徑），零 retry。compact strict 成功則不開新 ADR；只有放棄 strict 或改兩次呼叫才開。
