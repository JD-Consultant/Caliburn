...
完整本機 Web 先形成 release candidate；只有通過真實在職員工以本人工作完成的 R9 端到端 pilot gate，才稱為員工可用成品。
...
- [`specs/2026-06-27-system-architecture-design.md`](specs/2026-06-27-system-architecture-design.md) — **Historical foundation**：monorepo／契約優先／3 bounded context／Hexagonal+DDD仍保留；其中多租戶產品假設已被現行本機單人產品範圍取代。
- [`specs/2026-08-02-r0-architecture-truth-and-production-cutover-research.md`](specs/2026-08-02-r0-architecture-truth-and-production-cutover-research.md) — **R0 現行切換 authority**：實際 router／三代資料真相比對、Strangler Fig／ACL／Parallel Change／PostgreSQL 研究、以整份document單寫者並存、PV1 outcome、inventory／單向匯入、rollback與G0–G5退役gate；決策見ADR 0041。
...
- [`adr/0043-real-employee-pilot-release-gate.md`](adr/0043-real-employee-pilot-release-gate.md) — **真實員工試用發布門檻（Accepted，2026-08-03）**：R8是本機Web release candidate，R9必須由實際在職員工以本人工作完成端到端pilot；合成eval、高擬真transcript、內部自測與非操作者專家審閱不能取代，樣本與數值門檻於pilot前另核准。
...
- [`plans/2026-08-02-professional-consultant-production-cutover-plan.md`](plans/2026-08-02-professional-consultant-production-cutover-plan.md) — **現行 PV1／退役施工順序**：R0文件鎖、R1–R5前置gate、cross-language contract、current-row persistence、local_workspace、API/Web、inventory／可選importer、新文件cutover與三段legacy退役；每段列明測試與停線條件。
...
- [`design/`](design/) — **子系統端到端設計(agent-facing)**；現有 [`editor-knowledge-pack.md`](design/editor-knowledge-pack.md)、[`interview-engine.md`](design/interview-engine.md)（ACTIVE／TRANSITIONAL）與 [`professional-consultant-engine.md`](design/professional-consultant-engine.md)（R0切換邊界；新route未實作）。寫法見 [`design/README.md`](design/README.md)。
- [`adr/`](adr/) — Architecture Decision Records(決策的「為什麼」+ 取捨；**0001–0042**)；AI方向以[ADR 0040](adr/0040-professional-consultant-engine-and-r1-validation-contract.md)為準，production切換／退役以[ADR 0041](adr/0041-document-boundary-single-writer-cutover.md)為準，職務發現語意以[ADR 0042](adr/0042-hybrid-job-discovery-and-ttop-formation.md)為準。0038已被0040取代，勿據以開新工；0034的greenfield原則保留但operation catalog與state形狀不再沿用。
