...
- [`plans/2026-08-05-opks-progressive-elicitation-plan.md`](plans/2026-08-05-opks-progressive-elicitation-plan.md) — **已完成**：依 ADR 0054 把 OPKS 從按鈕觸發的一次性生成器接進主顧問訪談循環，八個 Phase 十八個 Task（domain／persistence 形狀 → pre-gate 純函式與主回合凍結唯一 child → gap 落地與四種終端 receipt → agenda 分區與 `issue_resolutions[]` → Current JD 清理 → `/turns` 接 child → 退役按鈕與三態呈現 → vertical 驗收）。決定 8「一個已提交的員工回合最多綁定一個 child」是最重要的不變量；不做百分比、queue、background worker、retry framework 或 agent router；live smoke 需 owner 當次授權。
- [`plans/2026-08-09-reviewed-job-analysis-slices-migration-plan.md`](plans/2026-08-09-reviewed-job-analysis-slices-migration-plan.md) — **待執行的選擇性移植計畫**：不合併未審遠端分支，依現行 ADR 0052–0056 在 progressive OPKS v3 主線上逐步重作 Header／readiness、Duty／Task 級別、OPKS 員工排序、嚴格單工作表公版 XLSX、Web cache／Proposal null 修正與安全診斷；十五個獨立 review gate，明確排除 16K token、其他 prompt 改寫、AI Duty、reference、server／enterprise 與舊 R1 runtime。
- [`plans/2026-08-10-legacy-cleanup-and-monorepo-boundary-plan.md`](plans/2026-08-10-legacy-cleanup-and-monorepo-boundary-plan.md) — **已完成**：inventory 確認沒有可安全刪除的 tracked asset；`job_analysis` API composition 與 Web current／legacy boundaries 已完成，最終 gates 與 zero-deletion 裁決記錄於 [`specs/2026-08-10-legacy-cleanup-inventory.md`](specs/2026-08-10-legacy-cleanup-inventory.md)。
...
- [`adr/`](adr/) — Architecture Decision Records(決策的「為什麼」+ 取捨;**0001–0056**)；**現行 AI 層方向以 [ADR 0040](adr/0040-professional-consultant-engine-and-r1-validation-contract.md) 為基礎，第一版 Context 邊界由 [ADR 0041](adr/0041-r1-p0-closure-first-version-context-and-holdout.md) 補充，時程風險接受與 A6 第一版預設以 [ADR 0042](adr/0042-r1-screening-stop-and-a6-first-version-default.md) 為準；本機 Current State persistence 與分層編輯 authority 見 [ADR 0043](adr/0043-job-analysis-local-current-state-persistence-and-authoring-authority.md)，不完整 JD Task 對齊見 [ADR 0044](adr/0044-partial-jd-task-reconciliation-and-human-confirmation.md)，Local Web 與共用 authority commit 見 [ADR 0045](adr/0045-job-analysis-local-web-contract-and-shared-authority-commit.md)，最小 AI 顧問迴圈見 [ADR 0046](adr/0046-professional-consultant-minimal-durable-loop.md)，模型自關 open issue 見 [ADR 0047](adr/0047-model-owned-open-issue-closure.md)**。provider 主線為 ADR 0035。0038、0039 已被取代，勿據以開新工。
  ...
  **OPKS 的「什麼時候分析、資料不夠怎麼辦」另見 [0054](adr/0054-opks-progressive-elicitation-and-scheduled-child-operation.md)（Accepted）**
  ——移除按鈕、主回合 receipt 凍結最多一筆 OPKS child、`uncertain` 承載缺口、`issue_resolutions[]`
  無副作用解 gap；0054 不翻案 0048–0051 的概念與持久化形狀。
