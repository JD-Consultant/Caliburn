...
- [`plans/2026-08-03-r1-task-discovery-offline-implementation-plan.md`](plans/2026-08-03-r1-task-discovery-offline-implementation-plan.md) — **R1 現行施工計畫**：T1離線 authority bundle 已實作，後續依序完成 prompt/schema＋runner、A1–A6 harness/Capture、blind grader/CLI/preflight，再在另有付費授權時做 live eval；未通過 gate 不進 R2。
...
- [`design/`](design/) — **子系統端到端設計(agent-facing)**；現有 [`editor-knowledge-pack.md`](design/editor-knowledge-pack.md)、[`interview-engine.md`](design/interview-engine.md)（ACTIVE／TRANSITIONAL）與 [`professional-consultant-engine.md`](design/professional-consultant-engine.md)（R1 T1 partial；新route未實作）。寫法見 [`design/README.md`](design/README.md)。
