...
- [`specs/2026-07-18-interview-vnext-llm-runtime-architecture-review.md`](specs/2026-07-18-interview-vnext-llm-runtime-architecture-review.md) — **vNext LLM runtime 2026 架構審查／下一版候選 authority**：交叉核對 OpenAI、Anthropic、Google、Microsoft、OpenRouter 與 OpenTelemetry 官方資料及 2026-07-18 真 live 診斷，確認 deterministic workflow + Context Engine + own LLM Port + provider adapters 主線，並提出 ProviderBinding/capability/conformance、route evidence、ID-less turn output、qualifier truth gate、C1/C2 ablation、Capture/OTel、文件共編與逐階 promotion；待 owner 核准後另立 ADR／實作 plan。
...
- [`adr/`](adr/) — Architecture Decision Records(決策的「為什麼」+ 取捨;**0001–0035**)；訪談 AI 核心架構為[ADR 0034](adr/0034-interview-ai-vnext-greenfield-evidence-workflow.md)，最新provider主線決策為[ADR 0035](adr/0035-interview-vnext-openrouter-first-provider-boundary.md)。
