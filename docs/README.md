...
- [`plans/2026-07-31-job-analysis-attributed-live-smoke-plan.md`](plans/2026-07-31-job-analysis-attributed-live-smoke-plan.md) — **待實作者執行的最小真模型驗證**：先以 no-network TDD 補 OpenRouter catalog／route／usage 證據與 budget guard，再用真 PostgreSQL、現行 `submit_employee_turn()` 與 exact Opus 5／Anthropic route 跑一次固定三回合；raw capture 留在 gitignored `output/`，Git 只收 bounded experiment report。
