...
- [`adr/0077-relational-jd-app-production-authority-and-pnpm-entrypoint.md`](adr/0077-relational-jd-app-production-authority-and-pnpm-entrypoint.md) — **Accepted；正式產品入口**：採用已驗關聯式 JD App，fresh PostgreSQL 18.6、Node 24＋pnpm 12 單一根入口；舊可執行架構硬退役但完整保留研究、設計、實驗與結果文件。
- [`plans/2026-09-10-jd-product-delivery.md`](plans/2026-09-10-jd-product-delivery.md) — **從現有成果到本機可用成品的歷史總計畫與長期驗收地圖**：原六切片進度與 Plate／三工具起點已被 relational JD、分層 Memory 及後續 G7 切片推進，不再把文末「Task 6」當目前唯一施工點。產品旅程、自然品質、正式採用與日常交付的長期 gate 仍可沿用；當前工作以 register 與下方 2026-09-20 整體證據契約為準。
- [`specs/2026-09-20-cross-agent-evidence-and-jd-context-contract.md`](specs/2026-09-20-cross-agent-evidence-and-jd-context-contract.md) — **A／B1／B2／C／Working State／JD 的整體證據與參數權責契約**：canonical 原話與正式 signed refs 保持原 owner；模型只撰寫語意內容，或選 Runtime 發出的短 `evidence_key`／既有 case、understanding、JD handles，不能自行產生 scope、版本、來源地址、ID 或保存結果。JD current targets 與同 view 續頁 cursor 保留原契約；Working State、Memory artifacts 與 JD source links 持久保存 resolved refs，不保存短 key；request-only compaction 不刪 private read proof。Task 0–5 的相鄰窄接線已完成；後續只做完整 App／自然 Luna／production authority gate，不新增 Agent、DB、Memory 或 provider。
- [`specs/2026-09-17-managed-app-background-callback-design.md`](specs/2026-09-17-managed-app-background-callback-design.md) — **JD-R002／MEM-L001 managed App 接線 G7 離線窄切片完成**：共用 A graph 無建構期 document reader；每次 invocation 注入兩文件隔離、每個 employee input 只讀一次的 availability provider。App-owned `BackgroundCoordinator` map 僅作 process-local single-flight，startup 在 Runtime ready 後才按 catalog＋durable state 恢復；一般 turn settle wake、shutdown drain 與 manual／no-key 無 coordinator 邊界保持。[§7 fresh evidence](specs/2026-09-17-managed-app-background-callback-design.md#7-g7-驗收與實際證據)保存當時證據。layered C 已由後續切片完成；本稿文末的舊「下一 gate 是 C」只屬當時施工順序。自然模型、完整瀏覽器旅程、production authority，以及多 process 才需要的原子 admission claim仍未完成，最新下一單位依 register／2026-09-20 整體證據契約。
...
- [`specs/2026-09-16-consultant-interview-working-state-design.md`](specs/2026-09-16-consultant-interview-working-state-design.md) — **JD-R002／CTX-W001 主顧問訪談工作面 G4 已對齊、尚待 G7**：從滿分 JD、完整工作分析與官方長任務設計反推最小欄位，讓 A 在第一輪尚無 Memory、一次問不完或尚未 compaction 時保存 Focus 與待追查事項。Stored state 與 Tool patch 分離，新增只必填三個核心語意欄位；它不是第二套 Memory、Agenda、固定問卷或 B1／B2 來源。來源 model-view 現依 2026-09-20 整體證據契約改用短 key，持久 state 仍保存 formal refs。
- [`specs/2026-09-22-jd-writing-skill-prompt-audit.md`](specs/2026-09-22-jd-writing-skill-prompt-audit.md) — **JD 撰寫方法 successor／短真模型與正式 16K gate 已通過**：日常訪談維持精簡；A 只有在實質撰寫、修訂或全面核對 JD 時，才按需讀一份 `write-customized-jd` Skill 與成文／完整性兩份 references。正式 Luna 驗收確認 compaction 後仍能依完整工作理解形成 O／P／K／S、保留未知與權責界線；同一主顧問、工具、Domain、A／B1／B2、Memory 與 compaction 不變，未新增 JD Agent 或平行規則。
- [`specs/2026-09-16-openrouter-continuation-compaction-design.md`](specs/2026-09-16-openrouter-continuation-compaction-design.md) — **JD-R002／CTX-C001 App-side 接線、角色化摘要及正式 16K 真 Luna gate 已完成**：維持單一 OpenRouter／Luna 與唯一 `ContinuationCompactionMiddleware`，以非破壞式 continuity summary 縮小 A／B1／B2 request；A 逐字保護最新員工訊息並可摘要其後已完成的舊工具 wave，B1 保留未處理訪談與案例差異／更正，B2 保留跨案例支持／反證並以固定任務作唯讀 orientation。canonical 原文、近期 opaque reasoning tail、Memory／JD authority 不變；舊 reasoning 不轉寫成摘要或第二套 Memory，不切 direct OpenAI、不等待原生 compaction item。17K 合成工作脈絡已通過正式 `medium／8192` 的摘要＋自然續作；長期真人訪談品質、每次 Luna reasoning metadata 與 provider-native compaction 仍不得由本 gate 推定。
- [`specs/2026-09-16-layered-case-and-work-understanding-memory-alignment.md`](specs/2026-09-16-layered-case-and-work-understanding-memory-alignment.md) — **JD-R002／MEM-L001 最新 Memory 產品語意與 G7 狀態**：B1 維護完整且可修正的案例／任務／事件與案例 guide，B2 維護穩定工作理解與理解 guide；兩層採同一 publication。Bundle、完整回合引用與兩層按需查找、B1／B2 staged Agent graphs、package 外層有界 rework／共同 publication／stale recovery、App dispatcher／正式資源、compaction、role factory、managed callback 與 layered C 均已完成各自離線切片。C 只處理已讀 latest、唯一且被使用者明確更正的既有 stable IDs；衝突未明、新案例／理解與廣泛重整仍交詢問、Working State 及 B1／B2。下一步是將同一短-key證據權責接到 A／Working State／JD，之後分開驗 provider／自然模型、完整瀏覽器旅程與 production authority。
...
- [`contract-strategy.md`](contract-strategy.md) — 契約來源、生成與交付的一般規範；正式 App 依其 `contracts/` 與生成核對執行。
- [`adr/README.md`](adr/README.md) — ADR 索引；0057 記錄 current-only hard cut。
- [`design/rag-pipeline.md`](design/rag-pipeline.md) — PDF → OCS contract → indexer → embedder/Qdrant 的 RAG 供應鏈；保留但與正式 JD App 完全隔離的獨立 bounded context，非產品 runtime。
