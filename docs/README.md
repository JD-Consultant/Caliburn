...
- [`specs/2026-07-29-job-analysis-partial-jd-task-reconciliation-research.md`](specs/2026-07-29-job-analysis-partial-jd-task-reconciliation-research.md) — **待書面複核的直接編輯對齊研究**：Current JD 只要求 Task 名稱／描述，其餘欄位可留空；不建假 Work Model Task、不用文字相似度猜 identity，改由 OpenIssue 明確持有 stable JD Task ID，後續訪談以同一 ID materialize Work Model Task，AI 對 JD 的改善仍須 Proposal。
