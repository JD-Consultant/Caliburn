...
- [`specs/2026-08-04-opks-progressive-elicitation-research.md`](specs/2026-08-04-opks-progressive-elicitation-research.md) — **待審**：OPKS 從一次性建議器改為會追問的顧問。核對 2026 官方資料（Anthropic workflow-vs-agent 與 `AskUserQuestion`、OpenAI agent guide 與 Deep Research clarifier、Temporal child workflow、Azure Durable Task at-least-once）與三篇 clarification 研究，收斂為 **application-controlled workflow（B）**：員工回合提交後由純函式 eligibility 最多追加一筆獨立 OPKS operation；`uncertain` 改為承載缺口摘要（**零 schema 變更**）、gap 不帶問句、單一 active question 不被覆寫、`analysis_input_digest` 編進 operation ID 以免重複付費、自動與手動 recovery 走不同 ID 政策。明確淘汰模型動態 routing／tool loop；含**被撤回主張表**與來源查證層級表。
