...
- [`specs/2026-07-27-professional-consultant-r1-task-discovery-experiment-design.md`](specs/2026-07-27-professional-consultant-r1-task-discovery-experiment-design.md) — **R1 Task Discovery 實驗設計與共識草案（Proposed）**：把ADR 0040／0041收斂為可實作的六arm比較，定義八案凍結、minimal/full Context、light/heavy schema、one/two-stage、公平的共同評審視圖、strongest/economical模型提案、experiment-only OpenRouter thin transport、三層capture、盲評與預先登記門檻；已經第二位審查者兩輪攻擊並形成共識，**狀態為Accepted for implementation**；Segment 1（八案凍結＋rubric＋契約＋離線verifier）已完成，實作分段見[`plans/2026-07-27-r1-task-discovery-implementation-plan.md`](plans/2026-07-27-r1-task-discovery-implementation-plan.md)。**Segment 4／5的付費live執行仍需owner另行核准。**
...
- [`adr/`](adr/) — Architecture Decision Records(決策的「為什麼」+ 取捨;**0001–0041**)；**現行AI層方向以[ADR 0040](adr/0040-professional-consultant-engine-and-r1-validation-contract.md)為準（Accepted，2026-07-26；完整取代0038），第一版Context與R1案例分層由[ADR 0041](adr/0041-r1-p0-closure-first-version-context-and-holdout.md)補充**；provider主線為[ADR 0035](adr/0035-interview-vnext-openrouter-first-provider-boundary.md)，本機多文件／公版樣式workspace為[ADR 0039](adr/0039-local-multi-document-canonical-public-form-workspace.md)（儲存語意段落由0040修正）。0034／0036／0037／**0038** 屬vNext路線：**0038已被0040取代，勿據以開新工**；0034的greenfield原則保留但operation catalog與state形狀不再沿用。
