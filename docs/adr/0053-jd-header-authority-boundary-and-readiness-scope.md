...
- 與 blind-first 顧問流程相容：固定開場仍是「先不用照職稱回答」，header 屬**待驗證脈絡**而非答案。
  ...
  **本切片不讓模型生成或提案 header**：header 由員工直接輸入或選擇，模型只讀不寫。
  可信分類資料的帶入仍依 [0052](0052-jd-readiness-assessment-and-official-code-boundaries.md)
  決定 9（`職業別`／`行業別` 可由分類資料帶入），與「模型不得生成」是兩件事。
