...
3. **Pre-gate 只擋明顯過早，不得宣稱資料完整**：Task 在 Current JD、Work Model Task 為 `ACTIVE`、
   有 ≥1 筆仍有效的員工依據、無指向該 Task 的未解邊界／矛盾／證據不足 issue、
   **無 active 的 OPKS gap**（定義見決定 20）、無 pending／deferred OPKS Proposal、
   **無相同 `analysis_input_digest` 的終端 receipt**（定義見決定 29）、
...
5. **不得以引文數、字數或涵蓋度加強 pre-gate。** 機械條件的誠實極限是「≥1 筆有效 SupportLink」；
   ...
   該 digest 的終端 receipt 使浪費上限為「每個輸入狀態一次呼叫」。
...
20. **active issue 的定義：`terminal_resolution is None`。**
    只有 active issue 能**進 agenda、取得 ordinal、被 `issue_resolutions[]` 指向、
    阻擋 OPKS pre-gate**。`terminal_resolution` 非空者留在 `open_issues` 內，
    但**僅作為「已問過、勿重問」的 context memory**：在 packet 另區呈現、**不配發 ordinal**，
    因此模型無法再次「解決」它，也不會讓它擋住新的分析。
21. **agenda 位置**：Task 邊界矛盾／責任問題 → 一般 open issue → **OPKS gap** → 遺漏掃描。

### E. 無副作用的解決通道

22. **新增與 `work_signals` 平行的第三個頂層陣列 `issue_resolutions[]`**，扁平、每 issue 一筆：
...
23. **`answered` 必須在同一輪有一筆與該 gap 的 `subject_task_id` 相關、且留下有效員工 Evidence
    的 `WorkSignal`**，否則 digest 不變、OPKS 不會再分析，gap 會被假關閉。
    這是機械可判的跨欄位條件，屬 Task Analysis verifier 的規則集。
24. **`answered` 維持現行語意（移出 `open_issues`）；`employee_unknown`／`not_applicable`
    不移除**，寫入 `terminal_resolution` 後依決定 20 轉為 context memory。
25. **resolution 的 `source_ref` 由 application 蓋，不進 wire。**

### F. Task 離開 Current JD 時的清理

26. **`prune_opks_for_current_jd()` 現行簽章只收／回 `CurrentJdOpks`，碰不到
    ...
    擴充該 seam 或加一個相鄰純函式。
27. **merge／split 一律終結 gap，不遷移。** 沿用既有政策「不把舊 refs 猜接到 replacement Task」。

### G. Receipt、失敗與付費邊界

28. **`OpksGenerationPayload` 新增 `analysis_input_digest` 與 `gap_issue_ids[]`；
    ...
    此一致性由 validator 寫死。
29. **四種 outcome 全部是終端 receipt，一律阻止相同 `analysis_input_digest` 自動重跑。**
    `failed` 由 terminal 失敗寫入（provider error／invalid output／refused／verifier rejected），
    與其餘三者在阻擋效果上等價，差別只在呈現與診斷。
    **abandon（digest 漂移）不寫 receipt，因此不阻擋**（決定 10）。
    **第一版不做 backoff、attempt counter、circuit breaker。**
30. **Proposal、OpenIssue 與 receipt 在同一個 `commit_authority_change()` 交易寫入。**
31. **成本上限**：每個已提交的員工回合最多一筆自動 OPKS call；相同 analysis input 不自動重跑；
    **零隱藏 retry**。
32. **Task Analysis 已提交後不因 OPKS 失敗而回滾**；`/turns` 不得因 child 失敗改回 5xx。
33. **不得宣稱 provider call at-most-once 或 exactly-once。** provider 已回應、Journal commit 前
    崩潰仍可能重打一次（外部呼叫的 at-least-once crash window）。Temporal 只能當概念類比，
    不能宣稱它替本 PostgreSQL＋HTTP 實作提供任何保證。

### H. UI 範圍與禁令

34. **`/turns` 最多執行主顧問＋一個 OPKS specialist**，員工只看到一個「分析中」。
35. **gap 不做側邊聊天、不做問題卡**，只由主顧問在同一聊天室問，一次一題。
36. **不做完成百分比、不宣稱「OPKS 已完整」、不做進度 dashboard。**
    狀態僅呈現「尚未適合分析／尚有待確認資訊／已可提出建議」，措辭受 0052 決定 7 約束。
37. **不引入通用 agent router、tool loop、handoff 或 tracing framework。**
...
- terminal issue 永久留在 `open_issues` 內（決定 20），狀態會隨訪談單調成長；
  第一版接受此代價，因為它只成長於員工真的答不出來的 gap，且不配發 ordinal、不進 agenda。
- `issue_resolutions[]` 給了模型一個新的出口，可能被當成偷懶關閉。決定 23 只能擋機械前提，
