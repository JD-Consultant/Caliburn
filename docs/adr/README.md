...
| [0049](0049-opks-derived-axes-evidence-whitelist-and-document-authority.md) | OPKS 施工前修正：`task_linkage`／`evidence_origin` 改為**由 refs 推導、不持久化**（比照 `Task.state`，消除「linked 但無 refs」等可表示的矛盾，並支援 mixed origin）；Evidence 來源白名單只收 `employee_turn`／`direct_edit`，**`proposal_decision` 不得為證據**（否則從後門放回 `employee_confirmed`），`reference` 等 Challenger 施工再加；文件層 K/S identity 採 ordinal 的 `reuse_existing`／`add_new`／`uncertain`＋員工在 Proposal 把關；**OPKS 不建平行 Work Model shadow**（真相只住 Current JD＋Proposal，拒絕記憶沿用既有 rejected Proposal）；OPKS 為獨立 operation 與獨立 wire schema；`deliverable_hint`／`success_criterion_hint` 退役；數值可查核性三檔、四體系樣本限定、taxonomy 理由改為證據保真 | **Accepted**（2026-08-01；修正 0048 決定 1–4／20／22／23 並補其未定義處；0048 概念裁決全部維持） |
| [0050](0050-opks-proposal-minimal-shape.md) | `OpksProposal` 最小形狀：**獨立、薄型別**（現行 Proposal 全是 Task 專用——action／target／`affected_task_ids`／`edited_jd_after` 皆綁 `TaskId`），不塞進 `ProposalTarget` 也不抽通用 framework；**第一版每項一筆**、以 `operation_id` 分組供 UI 呈現，不做批次核准；`entity_kind` 決定 refs 合法性（O/P 恰一個 task_ref，K/S 多對多，A 可空）；stale 三條規則；**只有員工真的改了 `after` 文字才鑄 direct-edit 來源**，單純 accept 不得偽造 | **Accepted**（2026-08-01；補充 0049 決定 8／11／13；closure review 放行條件之一） |
