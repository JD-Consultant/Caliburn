...
[`product-notes.md`](product-notes.md)與 [ADR 0057](adr/0057-server-deployed-browser-product.md)。完整 Web 先形成 release candidate；
...
- [`specs/2026-07-30-professional-consultant-minimal-complete-loop-research.md`](specs/2026-07-30-professional-consultant-minimal-complete-loop-research.md) — **已完成 strongest-case 反方審查的專業顧問最小完整迴圈設計**：沿用 A6 強模型＋light schema＋one-stage，以固定持久開場、全訊號理解、單一最高資訊價值下一題、Proposal 與訪談並行及三個薄 API 接通可關閉重開的 AI Task 訪談；補上 provider 前 replay short-circuit 與完整 Proposal 歷史，暫不新增 Role/Coverage/Story、Graph、多 Agent、streaming 或 O/P/K/S/A。
- [`specs/2026-07-31-job-analysis-openrouter-attribution-and-live-smoke-research.md`](specs/2026-07-31-job-analysis-openrouter-attribution-and-live-smoke-research.md) — **現行 Job Analysis 真 OpenRouter 路徑的最小驗證研究**：以 2026-07-31 OpenRouter／Anthropic／OpenAI 官方資料與 live catalog 核對 exact Opus 5／Anthropic endpoint，採 additive router metadata＋單一三回合 synthetic product smoke；最多三次 call、US$0.75、無 retry／grader／矩陣，不把 telemetry 寫入產品真相，也不以單次結果宣稱 Task Discovery 通過。執行結果見
  [`experiments/2026-07-31-job-analysis-attributed-live-smoke/README.md`](experiments/2026-07-31-job-analysis-attributed-live-smoke/README.md)
  — **turn 1 停線，1 次 call、US$0**：Anthropic 以 HTTP 400 `invalid_request_error` 拒絕請求
  （"The compiled grammar is too large"），失敗在 **strict output schema（6,818 bytes）而非 prompt 或 Context**（packet 僅 551 字元）；
  route 本身正常（direct、attempt 1、無 pipeline）。沒有任何模型輸出，因此**不得推得任何品質結論**；
  production 走同一份 schema，同 route 下的員工 AI 回合現在應同樣失敗。修正方向須先開研究與 ADR（動到 ADR 0040 決定 26）。
- [`specs/2026-07-31-context-engineering-model-facing-contract-research.md`](specs/2026-07-31-context-engineering-model-facing-contract-research.md) — **context 該放什麼：主流做法與 Caliburn 對照**：依 Anthropic context engineering、Claude 5 世代新規則（Claude Code 砍掉 >80% system prompt、coding eval 無可測損失）、Opus 5 prompting、structured outputs、writing tools、building effective agents 與 OpenAI 對照組，收斂六條原則；量出我們的請求 **90% 是每回合不變的固定成本**（instructions 43%＋schema 47%，動態 packet 僅 6%），其中 prompt 有 **2,336 bytes（37.6%）在複述 verifier 已在跑的 28 個 violation code**、schema 有 **1,868 bytes（27.4%）是 Pydantic 自動 `title` 與含 `§12.3`／`TI-R1-08` 內部代號的開發者 `description`**；`limitations`／`next_question.purpose` 零消費者，`context`／`deliverable_hint`／`success_criterion_hint` 只回流 packet 自己（成本 O(N tasks×T turns×2)）。提出「語意獨有＋有真實下游消費者＋錯了抓得到」三判準加「罕見路徑不得讓常見路徑付固定成本」，並依官方「個別限制全滿足仍可能被拒、另有未公開 grammar size 上限」**建議取消 Probe U**（結果不改變下一步），把該次 live 驗證留給精簡後的 production schema。
- [`specs/2026-07-31-anthropic-strict-schema-grammar-limit-research.md`](specs/2026-07-31-anthropic-strict-schema-grammar-limit-research.md) — **strict schema grammar 上限查證與修法分析**：Anthropic 官方明列 optional 24／union parameters 16（strict tool use 與 structured outputs 共用編譯管線，故錯誤訊息誤稱 tool）。本產品 schema 送出時 13 union、**本地 `$ref` 展開後 17 → 超出 16**，是目前最強、可直接否證的根因**假說**（400 只回報 grammar too large，未回報 union count，因果待驗證）；optional 17 在上限內，但官方限制可能互相作用，尚未排除。方向為「零 `anyOf` 的中性值 wire schema ＋ 純 mapper」，先以丟棄式 Probe U（只做 `anyOf:[X,null] → X`、單次上限 US$0.05、零 retry、不進 application transition）驗證；通過後仍須為最終 production schema 單獨重跑 turn 1。strict 若能維持產品語意則不開新 ADR。
...
- [`specs/2026-08-03-r1-t3-ablation-manifest-capture-research.md`](specs/2026-08-03-r1-t3-ablation-manifest-capture-research.md) — **R1 T3 實作研究**：依 Pydantic／Python／OpenRouter 官方文件固定 literal 六臂、planned 48/80 與 actual evidence 分離、三層 canonical artifact＋SHA-256 reference、create-only Trial Manifest 與 response-resolved facts；blind grader／CLI／live preflight 留在 T4。
- [`specs/2026-08-04-r1-t4-blind-grader-cli-openrouter-preflight-research.md`](specs/2026-08-04-r1-t4-blind-grader-cli-openrouter-preflight-research.md) — **R1 T4 實作研究**：依 OpenRouter Structured Outputs／Provider Routing／Router Metadata／Generation 官方文件與 Pydantic／Python 標準庫固定 blind grader 獨立 port／盲化投影邊界、CLI dry-run 邊界，以及 preflight「只 attest、不猜測」的 resolved facts fail-closed policy；三個切片各一 commit。
...
- [`adr/0046-professional-consultant-minimal-durable-loop.md`](adr/0046-professional-consultant-minimal-durable-loop.md) — **專業顧問最小 durable Task 訪談迴圈（Accepted，2026-07-30）**：沿用 A6 單一 one-stage 顧問，固定持久開場、application-owned conversation、Proposal 與下一題並行及三個薄 route；provider 前短路 committed replay，暫不新增 Role/Coverage、Graph、多 Agent、streaming 或 completion gate。
...
- [`adr/0054-document-boundary-single-writer-cutover.md`](adr/0054-document-boundary-single-writer-cutover.md) — **production cutover／退役決策（Accepted，2026-08-02）**：以整份`document_id`為切換單位、同一文件單一writer；v3 maintenance-only；R1–R5過gate才掛PV1；禁止OCS/current-row dual-write；inventory／單向匯入後分task移除舊Web、route、code與table。
- [`adr/0055-hybrid-job-discovery-and-ttop-formation.md`](adr/0055-hybrid-job-discovery-and-ttop-formation.md) — **混合式職務發現決策（Accepted，2026-08-03）**：暫定職務框架與reference只作假說；開放敘事加定向補漏、跨敘事形成Task後歸納Duty，Task穩定後共同定義O/P，KSA須有linkage／支持度並經人確認。
- [`adr/0056-real-employee-pilot-release-gate.md`](adr/0056-real-employee-pilot-release-gate.md) — **真實員工試用發布門檻（Accepted，2026-08-03）**：R8是Web release candidate，R9必須由實際在職員工以本人工作完成端到端pilot；合成eval、高擬真transcript、內部自測與非操作者專家審閱不能取代。其localhost部署字句由ADR 0044取代，真人gate保留。
- [`adr/0057-server-deployed-browser-product.md`](adr/0057-server-deployed-browser-product.md) — **Server-deployed browser product（Accepted，2026-08-03）**：企業自管或我們代管同一套單企業deployment；R8補production Compose／TLS／維運gate，員工電腦localhost假設失效，共享多租戶與identity另案。
...
- [`plans/2026-07-30-job-analysis-local-web-first-slice-plan.md`](plans/2026-07-30-job-analysis-local-web-first-slice-plan.md) — **核心切片已完成，browser 點擊待環境確認**：已收斂 authority commit seam，接通 JSON Schema 契約、文件／Task API、本機 Web 文件庫、明確儲存 Task editor 與真實 PostgreSQL／HTTP/CORS；不含 AI UI、舊資料整合、autosave 或 ETag。2026-07-30 執行環境沒有可連接瀏覽器，未以元件測試冒充實際點擊。
- [`plans/2026-07-30-professional-consultant-minimal-loop-plan.md`](plans/2026-07-30-professional-consultant-minimal-loop-plan.md) — **最小專業顧問閉環已完成**：固定持久開場、lossless conversation Journal、provider 前 replay、one-stage 彈性顧問 prompt、Consultation／Proposal wire contract、三個薄 routes 與 conversation／Proposal／Current JD 同頁 Web；不接舊系統、不新增 planner、Graph、多 Agent、completion gate 或新持久表。
- [`plans/2026-07-31-job-analysis-attributed-live-smoke-plan.md`](plans/2026-07-31-job-analysis-attributed-live-smoke-plan.md) — **已執行完成的最小真模型驗證**（2026-07-31 Opus 5／Luna-Pro／Sonnet 5 三回合，2026-08-02 另有兩次 OPKS 單次呼叫；紀錄見 `experiments/`）：先以 no-network TDD 補 OpenRouter catalog／route／usage 證據與 budget guard，再用真 PostgreSQL、現行 `submit_employee_turn()` 與 exact Opus 5／Anthropic route 跑一次固定三回合；raw capture 留在 gitignored `output/`，Git 只收 bounded experiment report。
- [`plans/2026-08-01-job-analysis-opks-first-slice-plan.md`](plans/2026-08-01-job-analysis-opks-first-slice-plan.md) — **已完成的 OPKS 第一切片**：依 ADR 0048–0051 以薄 `OpksItem`／獨立 `OpksProposal` 接通 Current JD persistence、員工手動編輯、單 Task O/P/K/S 提案、Web 與七情境 PostgreSQL scripted vertical；不做匯出、taxonomy、Reference Challenger、自動判級、Attitude AI 或通用 Proposal framework。scripted provider 只驗管線，不代表真模型品質。
...
- [`plans/2026-08-03-r1-task-discovery-offline-implementation-plan.md`](plans/2026-08-03-r1-task-discovery-offline-implementation-plan.md) — **R1 現行施工計畫**：T1 authority、T2 prompt/schema/runners、T3 A1–A6/minimal baseline/immutable capture harness、T4a 獨立 blind grader已實作；下一步做 T4b OpenRouter preflight seam 與 T4c CLI，另有付費授權時才做 live eval；未通過 gate 不進 R2。
...
- [`adr/`](adr/) — Architecture Decision Records(決策的「為什麼」+ 取捨;**0001–0057**)；**現行 AI 層方向以 [ADR 0040](adr/0040-professional-consultant-engine-and-r1-validation-contract.md) 為基礎，第一版 Context 邊界由 [ADR 0041](adr/0041-r1-p0-closure-first-version-context-and-holdout.md) 補充，時程風險接受與 A6 第一版預設以 [ADR 0042](adr/0042-r1-screening-stop-and-a6-first-version-default.md) 為準；本機 Current State persistence 與分層編輯 authority 見 [ADR 0043](adr/0043-job-analysis-local-current-state-persistence-and-authoring-authority.md)，不完整 JD Task 對齊見 [ADR 0044](adr/0044-partial-jd-task-reconciliation-and-human-confirmation.md)，Local Web 與共用 authority commit 見 [ADR 0045](adr/0045-job-analysis-local-web-contract-and-shared-authority-commit.md)，最小 AI 顧問迴圈見 [ADR 0046](adr/0046-professional-consultant-minimal-durable-loop.md)，模型自關 open issue 見 [ADR 0047](adr/0047-model-owned-open-issue-closure.md)**。provider 主線為 ADR 0035。0038、0039 已被取代，勿據以開新工。
  ...
  **[ADR 0040 決定 29–30 的「支持度四級」已被 0048 翻案**，凡文件寫「四級支持度」者（**包含本檔上方的 R1／0040／0042 摘要**）皆為**歷史紀錄**，不得據以施工。
  ...
  與五份 `specs/2026-08-01-opks-raw-*.md` 原料。**產品交付邊界以 [ADR 0057](adr/0057-server-deployed-browser-product.md) 為準**（server-deployed browser，取代先前 localhost 產品措辭）；第一版發布門檻見 [ADR 0056](adr/0056-real-employee-pilot-release-gate.md)，職務發現語意見 [ADR 0055](adr/0055-hybrid-job-discovery-and-ttop-formation.md)，文件單寫者切換見 [ADR 0054](adr/0054-document-boundary-single-writer-cutover.md)。0054–0057 原以 0041–0044 起草於離線分支，合併時因編號已被佔用而重新編號。
