...
> 2026-09-07 · **G3：Owner 已核准 §4 方案 A；實作與驗證另見[窄修結果](2026-09-07-b2-delivery-repair-results.md)，尚未自然 Memory 品質驗收。**
...
| 選項 | 效果與風險 | 成本／接線 |
|---|---|---|
| **A：既有工具，改操作策略＋完成檢查（Owner 已核准隔離窄修）** | 小檔已完整讀取可用官方 write 一次更新該檔，不必反覆抄 old_string；長正文仍局部 edit。最後同時核對正文／導覽；保留有效 no-op。仍可能漏內容／誤判，不能以機制通過替代品質驗收。 | 不增依賴／新 wire；小檔輸出可能多於精準 patch，但可少掉錯誤往返。是否實際較便宜待原樣本；不先提額。 |
...
**A 的短設計（2026-09-07 Owner 已核准；B/C 仍是備選）：**
...
**Finding：**不能只靠加 prompt；完成契約有可定位缺口，官方檔案工具的選用也未必適合所有檔案大小。**Status：G2 完成，G3 已核准 A；施工／測試狀態由[窄修結果](2026-09-07-b2-delivery-repair-results.md)持有。**MP-02d/e 與自然整組品質仍 OPEN。研究本身沒有付費／改碼；後續 A 僅授權隔離窄修，既有原文／詳記／引用與發布責任不翻案，patch／輸出契約變更仍需新 gate。
