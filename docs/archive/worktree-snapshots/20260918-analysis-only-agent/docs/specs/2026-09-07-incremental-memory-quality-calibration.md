...
## Owner 後續澄清：要保留的是職務分析有用的細節

2026-09-07，Owner明確指出「細節」指工作案例中可用來判斷員工工作／任務、工作頻率及後續追問的重要資訊，不是所有瑣事都要同等保存。這是保留目的的澄清，不回寫上述已完成實驗的結果，也尚未修改prompt或新增測試。

- 以既有職務分析方法判斷相關性：本人實際做什麼、目的與結果、責任／決策／交接界線、頻率／觸發條件、例行或偶發性、重要限制與例外、品質判準及實際所需知識技能等。這些是判斷面向，不是新的必填Memory schema或封閉欄位清單。
- 案例保留能說明共同工作及重要差異的細節；可以歸納、去重、換句話說，不要求逐字維持每一行。無關閒聊、重複敘述及不影響工作判斷的枝節，不必進入詳記／長期理解；原始對話保存政策不因此改變。案例名稱等若有助後續定位，仍可保留，不一律當作無用資訊。
- 「這輪未提及不等於撤銷」仍適用於先前成立的**工作相關資訊**，不是無限制累積所有文字。低頻不等於無關；未確定相關性也不能直接等同沒有價值。原測試遺失主管查看權限涉及角色／權限界線，仍是有效問題。
- 方法來源：回看[R1研究 §2.2、§5](../../../../docs/specs/2026-07-25-professional-consultant-r1-task-discovery-deep-research.md)的案例≠任務、角色責任、產出、頻率／重要性判準；只借分析方法，不沿用其舊架構決策。另核對目前[工作範圍訪談](../../experiments/analysis-agent/src/analysis_agent/skills/work-scope-interview/SKILL.md)、[共同工作模式](../../experiments/analysis-agent/src/analysis_agent/skills/compare-work-patterns/SKILL.md)、[產出與專業](../../experiments/analysis-agent/src/analysis_agent/skills/outcomes-and-expertise/SKILL.md)三份Skill。

下一次提示品質校準應檢查「是否保住職務分析所需資訊、排除無關瑣事」，不能只以原文字數、字面不變或全部細節照存當成品質目標。本輪僅記錄並確認理解，CT05-Q01待驗狀態不變。
