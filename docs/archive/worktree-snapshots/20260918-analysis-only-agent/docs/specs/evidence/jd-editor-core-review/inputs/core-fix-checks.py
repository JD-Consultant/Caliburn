...
kind = sys.argv[1]
if kind == 'python':
    checks = [('python', [sys.executable, 'tests/task5_acceptance_runner.py', '-q', 'tests/test_jd_shutdown_isolation.py', 'tests/test_service.py', 'tests/test_conversation_lifecycle.py', 'tests/test_read_recovery.py', '--basetemp=../../scratch/core-fix-python-temp'], A),
              ('pg', [sys.executable, str(HERE/'task4-run.py'), sys.executable, 'tests/task5_acceptance_runner.py', '-q', 'tests/test_jd_close_reconcile.py', 'tests/test_jd_admission.py', 'tests/test_jd_postgres_recovery.py', '--basetemp=../../scratch/core-fix-pg-temp'], A)]
else:
...
for name, argv, cwd in checks:
    started = monotonic()
    with (ROOT/f'scratch/core-fix-{name}.log').open('w', encoding='utf8') as out:
    ...
    results.append(item)
    (ROOT/f'scratch/core-fix-{kind}-results.json').write_text(json.dumps(results, indent=2), encoding='utf8')
