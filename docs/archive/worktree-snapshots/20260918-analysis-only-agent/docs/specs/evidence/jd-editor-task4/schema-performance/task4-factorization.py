...
ROOT=HERE.parents[2]
OUT=HERE/'task4-schema-candidate'
...
for name in composed:
    ...
    for branch in branches:
        wrapper=defs[branch['$ref'].split('/')[-1]]
        assert len(wrapper['allOf'])==2 and wrapper['allOf'][0]=={'$ref':'#/$defs/JdSavedElement'}
    defs[name]={'allOf':[{'$ref':'#/$defs/JdSavedElement'}, {'oneOf':[{'$ref':b['$ref']+'/allOf/1'} for b in branches]}]}
changed=[]
def factor(value):
...
def compare(label,name,value,expected=None):
    answer=[]
    for schema in (old,new):answer.append(validator(schema,name).is_valid(value))
...
try:
    ...
    for name in wrappers:
        ...
        valid=next(n for n in eligible if validator(old,name).is_valid(n))
    ...
    timing=[]
    for label,schema in [('before',old),('candidate',new)]:
