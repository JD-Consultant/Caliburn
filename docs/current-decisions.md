...
| `MEM-Q004` read contract／isolated spike design | [`specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md`](specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md) | **G4 complete／兩次 bounded repair verified**；§13 的 Runtime-owned ID＋append-only exposed boundary，以及 §14 的 OpenRouter SDK operation-wrapper 讀取均已完成 TDD 與 deterministic verification；read shape 與 index 邊界不變，不授權 production 或 live rerun。 |
| `MEM-Q004` isolated spike plan | [`plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md`](plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md)、[`specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md`](specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md) | **Live preflight stopped／harness repair verified**；唯一獲准 attempt 尚未發 embedding／Luna call即因 SDK wrapper 接線錯誤停止。修復後 dry-run 27 passed、完整 deterministic suite 59 passed／1 optional skip，另以臨時 LangMem 0.0.30 跑該 characterization 1 passed。下一個 gate 是 Owner 決定是否建立新 trial revision；不得自行重跑、改 rubric 或接 production。 |
...
```text
Topic ID: MEM-Q004
Current stage: G5 live preflight 已停止；bounded harness repair 已驗證，尚未授權 live rerun
Binding decisions: MEM-D000～MEM-D003、MEM-Q001～MEM-Q004
This turn's only blocking question:
  是否建立新的 trial revision 並重跑唯一一次 Luna smoke。
  本輪 bounded repair 已完成；不得把修復本身解讀為 live rerun 授權。
Already reviewed evidence:
  ...
  的 `endpoints.list_async()` 回傳 `operations.ListEndpointsResponse(data=...)`，但 harness 誤讀
...
`MEM-Q003` 已完成；`MEM-Q004` 的 read shape、isolated semantic-index mechanism、Tool 名稱與最小 contract 均已完成 G4 收斂。唯一獲准 live attempt 已在 metadata preflight 因 harness／pinned SDK contract 接線錯誤停止；bounded repair 已完成 TDD 與 deterministic verification。下一步只能由 Product Owner 決定是否建立新 trial revision；目前不得重跑，也不授權 production、merge 或 push。
