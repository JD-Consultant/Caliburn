...
| `MEM-Q004` isolated spike plan | [`plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md`](plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md)、[`specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md`](specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md) | **Trial revision 2 authorized**；revision 1 尚未發 embedding／Luna call即因實驗執行器 SDK wrapper 接線錯誤停止。修復後 dry-run 27 passed、完整 deterministic suite 59 passed／1 optional skip，另以臨時 LangMem 0.0.30 跑該 characterization 1 passed。Owner 已核准以完全不變的案例、prompt、rubric、模型與成本／call caps 執行 revision 2；結果無論 PASS／FAIL 都保存並停止，不得第三次執行或接 production。 |
...
```text
Topic ID: MEM-Q004
Current stage: G5 trial revision 2 已授權，等待依 frozen contract 執行
Binding decisions: MEM-D000～MEM-D003、MEM-Q001～MEM-Q004
This turn's only blocking question:
  無；Product Owner 已在理解隔離 smoke 實驗執行器與 agent runtime 的差別後，
  核准以 trial revision 2 重跑同一個 Luna smoke。不得修改 frozen case／prompt／rubric 或原 caps。
...
`MEM-Q003` 已完成；`MEM-Q004` 的 read shape、isolated semantic-index mechanism、Tool 名稱與最小 contract 均已完成 G4 收斂。trial revision 1 在 metadata preflight 因實驗執行器／pinned SDK contract 接線錯誤停止，bounded repair 已完成 TDD 與 deterministic verification；Product Owner 已核准執行 trial revision 2。執行後無論結果都必須保存並停止；目前不授權 production、merge 或 push。
