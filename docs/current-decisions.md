...
| `MEM-Q004` | `WORKING` | G4 read contract 採 progressive disclosure：先由小型、可重建的 Semantic Memory 導覽定向；需要更多內容時，模型使用 `search_semantic_memory(query)`，Runtime 在目前 JD／文件 scope 內回傳少量但逐筆完整、自成一體的 current Semantic Memory，以及輕量 `message_refs[]`；只有需要核對原句或問答脈絡時才以 `read_conversation_context(message_ref)` 深讀 canonical conversation。兩個 Tool 都只有一個 required string；scope／limit／filter／窗口／retry 由 Runtime 管理。第一版不另建 conversation summary。Isolated spike 只替 focused Semantic Memory 啟用 LangGraph Store semantic index／embedding；raw conversation semantic index 維持 deferred。canonical message ID 由可信 Runtime 建立；產品只開放 append，不讓模型／員工／Web 提供既有 ID 或 update／delete 歷史，也不加入 same-ID content comparison guard。名稱、最小結果視圖與 append-only exposed boundary 是 Caliburn mapping，不冒充 vendor 標準。 | Product Owner 2026-09-03 核准 read shape、isolated semantic-index mechanism、兩個 Tool 名稱、trial revision 2 與三次 bounded repair。Revision 2 在 metadata preflight 後因 Windows CLI 預設 `ProactorEventLoop` 不受 Psycopg 支援而停止；沒有 embedding／Luna／tool call 或模型語意輸出，結果維持 `FAIL_UNPROVEN`。其後只獲准的 event-loop repair 已以真 CLI＋Psycopg RED→GREEN 驗證，未重跑模型。Product Owner 於 2026-09-04 接著核准一次新的 **live trial revision 3**：frozen case／prompt／rubric、Luna medium、embedding model、call／tool／token／cost caps 與零 retry 全部不變；不論結果均保存並停止。官方、框架與實際證據見 [`specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md`](specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md) §3、§6、§13～§17 與[實驗報告](experiments/2026-09-03-memory-routing-canonical-read/report.md)。 | Isolated spike 證明自然語言 routing、canonical deep-read、scope isolation、成本或延遲不可接受；產品邊界無法阻止 caller 指定既有 ID；官方 primitive 改變；或新 live evidence 改變結論。 | 先取得「允許將匿名 synthetic frozen fixture／prompt／tool results 傳送至 OpenRouter」的明確外部傳輸授權；取得後只執行一次 revision 3，保存並回到 Owner review。不授權 revision 4、production、merge 或 push。 |
...
| `MEM-Q004` read contract／isolated spike design | [`specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md`](specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md) | **G4 complete／live trial revision 3 authorized**；§13 Runtime-owned ID、§14 SDK wrapper 與 §16 Windows CLI event-loop 均已完成 bounded repair。§17 記錄 frozen revision 3 授權；read shape 與 index 邊界不變，不授權 production。 |
| `MEM-Q004` isolated spike plan | [`plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md`](plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md)、[`specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md`](specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md) | **Revision 2 FAIL_UNPROVEN／runner repaired；revision 3 authorized**。原 revision 2 artifact 與 verdict 不變；新 attempt 只可執行一次，保存後停止，不授權 revision 4 或 production。 |
...
```text
Topic ID: MEM-Q004
Current stage: G5 live trial revision 3 產品 gate 已核准；外部傳輸安全 gate 尚待明確授權，未執行
Binding decisions: MEM-D000～MEM-D003、MEM-Q001～MEM-Q004
This turn's only blocking question:
  是否明確允許將匿名 synthetic frozen fixture、prompt 與執行中產生的 tool results 傳送至 OpenRouter，
  以執行一次最高 USD 0.20、零 retry 的 Luna medium revision 3。
...
`MEM-Q003` 已完成；`MEM-Q004` 的 read shape、isolated semantic-index mechanism、Tool 名稱與最小 contract 均已完成 G4 收斂，但 live 模型能力仍未證明。Trial revision 2 的 `FAIL_UNPROVEN` 報告維持不變；Windows CLI event-loop bounded repair 已完成真 Psycopg RED→GREEN。Product Owner 已核准 frozen live trial revision 3 的產品 gate，但安全 gate 要求另行明確授權把匿名 synthetic 測試內容傳至 OpenRouter；目前尚未執行，也不授權 revision 4、production、merge 或 push。
