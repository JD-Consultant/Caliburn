...
| `MEM-Q004` | `WORKING` | G4 read contract 採 progressive disclosure：先由小型、可重建的 Semantic Memory 導覽定向；需要更多內容時，模型使用 `search_semantic_memory(query)`，Runtime 在目前 JD／文件 scope 內回傳少量但逐筆完整、自成一體的 current Semantic Memory，以及輕量 `message_refs[]`；只有需要核對原句或問答脈絡時才以 `read_conversation_context(message_ref)` 深讀 canonical conversation。兩個 Tool 都只有一個 required string；scope／limit／filter／窗口／retry 由 Runtime 管理。第一版不另建 conversation summary。Isolated spike 只替 focused Semantic Memory 啟用 LangGraph Store semantic index／embedding；raw conversation semantic index 維持 deferred。canonical message ID 由可信 Runtime 建立；產品只開放 append，不讓模型／員工／Web 提供既有 ID 或 update／delete 歷史，也不加入 same-ID content comparison guard。名稱、最小結果視圖與 append-only exposed boundary 是 Caliburn mapping，不冒充 vendor 標準。 | Product Owner 2026-09-03 核准 read shape、isolated semantic-index mechanism、兩個 Tool 名稱與兩次 bounded repair，並核准 trial revision 2。Revision 2 在 metadata preflight 後因 Windows CLI 預設 `ProactorEventLoop` 不受 Psycopg 支援而停止；沒有 embedding／Luna／tool call 或模型語意輸出，結果為 `FAIL_UNPROVEN`。官方、框架與實際證據見 [`specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md`](specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md) §3、§6、§13～§16 與[實驗報告](experiments/2026-09-03-memory-routing-canonical-read/report.md)。 | Isolated spike 證明自然語言 routing、canonical deep-read、scope isolation、成本或延遲不可接受；產品邊界無法阻止 caller 指定既有 ID；官方 primitive 改變；或 Windows runner repair 後的實證改變結論。 | Revision 2 已保存並停止；下一個 gate 是 Product Owner review report，另行決定是否只核准 event-loop bounded repair。不授權第三次 live attempt 或 production。 |
...
| `MEM-Q004` read contract／isolated spike design | [`specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md`](specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md) | **G4 complete／live evidence unproven**；§13 Runtime-owned ID、§14 SDK wrapper 均已修復驗證；§16 記錄 Windows CLI event-loop entrypoint 缺口。read shape 與 index 邊界不變，不授權 production。 |
| `MEM-Q004` isolated spike plan | [`plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md`](plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md)、[`specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md`](specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md) | **Revision 2 executed／FAIL_UNPROVEN**；frozen contract 未變，但 attempt 在第一個 Psycopg connection 前停止，沒有 embedding／Luna／tool call 或模型輸出。attempt record、results 與 report 已保存；不得第三次執行或接 production。 |
...
```text
Topic ID: MEM-Q004
Current stage: G5 trial revision 2 已保存為 FAIL_UNPROVEN，等待 Product Owner report review
Binding decisions: MEM-D000～MEM-D003、MEM-Q001～MEM-Q004
This turn's only blocking question:
  是否只核准 Windows CLI event-loop bounded repair。即使 repair 通過，也不自動授權新的 live attempt。
Already reviewed evidence:
  ...
  loop_factory 配置。DB-only、零 provider request 診斷已在同環境成功，但 repair 尚未授權。
...
`MEM-Q003` 已完成；`MEM-Q004` 的 read shape、isolated semantic-index mechanism、Tool 名稱與最小 contract 均已完成 G4 收斂，但 live 模型能力仍未證明。Trial revision 2 已因 Windows CLI event-loop plumbing failure 如實保存並停止。下一步只能由 Product Owner review report 並決定是否核准 bounded runner repair；目前不授權第三次 live attempt、production、merge 或 push。
