...
| `MEM-Q004` read contract／isolated spike design | [`specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md`](specs/2026-09-03-memory-routing-canonical-read-and-isolated-spike-research.md) | **G4 complete／Revision 3 repair verified**；read shape、focused Semantic Memory semantic index、兩個 Tool 名稱與最小 contract 已核准，raw conversation index 維持 deferred；§13 的 Runtime-owned ID＋append-only exposed boundary 已完成 isolated bounded repair、deterministic verification 與 review；不授權 production。 |
| `MEM-Q004` isolated spike plan | [`plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md`](plans/2026-09-03-memory-routing-canonical-read-isolated-spike.md)、[`specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md`](specs/2026-09-03-memory-read-spike-consensus-and-framework-final-audit.md) | **Revision 3 bounded repair complete／single live smoke authorized**；已移除 framework same-ID update 上方的自訂 content guard，由 trusted Runtime／fixture 建立 fresh IDs；TDD 與完整 deterministic suite 為 58 passed、1 optional skip，獨立 review 無 finding。Owner 已核准唯一一次凍結的 Luna smoke；不得 retry、改 rubric 或接 production。 |
...
```text
Topic ID: MEM-Q004
Current stage: G5 Revision 3 bounded repair 已完成；唯一一次 Luna live smoke 已授權，正在 preflight／execution
Binding decisions: MEM-D000～MEM-D003、MEM-Q001～MEM-Q004
This turn's only blocking question:
  無；Product Owner 已核准依凍結 fixture、prompt、model/tool/token/cost caps 執行一次 Luna medium smoke。
  下一個 blocking gate 是實際 report review；不論 PASS／FAIL 都不得自行重跑或接 production。
...
`MEM-Q003` 已完成；`MEM-Q004` 的 read shape、isolated semantic-index mechanism、Tool 名稱與最小 contract 均已完成 G4 收斂，Revision 3 bounded repair 也已完成 deterministic verification 與獨立 review。Product Owner 已明確核准執行唯一一次已凍結的 Luna smoke；執行後必須如實寫 report 並回到 Owner review，不得重跑，也不授權 production、merge 或 push。
