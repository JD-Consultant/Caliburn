...
| JD header | `JdHeader`：iCAP 版型表頭語意欄位（職能基準名稱／所屬類別／工作描述／nullable 基準級別 1–6／說明補充） | `app/job_analysis/domain/jd_header.py` | 純 Pydantic,frozen;全欄位 nullable,空字串拒收(要 `null` 不要 `""`);**沒有 `職能基準代碼`／`職類別代碼` 欄位**(iCAP 配發,不生成)。**T6 已落地**:route、wire contract、Task Analysis packet 與 `/workspace` 編輯表單都已接通,員工可實際填寫 |
| Duty（主要職責） | `Duty`：Current JD 的第一層結構；Task 另有 nullable `duty_id` 與 1–6 `competency_level` | `app/job_analysis/domain/duty.py`、`JdTask` | 官方建議只做主要職責→工作任務**兩層**，不做工作活動第三層。`Duty` **刻意沒有**級別（級別掛 Task，因為一條 Duty 可涵蓋不只一個級別）、沒有 `T1` 位置碼（匯出時依排序決定性產生，不落庫）、沒有 purpose／outcome（官方版型沒有）。未指派的 Task 是合法狀態——ADR 0052 決定 13 禁止合成假的 T1。「職責大小是否一致」是語意判斷，不進 verifier |
| readiness | `assess_readiness(header=…, duties=…, tasks=…)` → 只有 issue 清單的 `DocumentReadiness` | `application/readiness.py` | 純函式,零 IO,**不 import transport contract**;查表頭三項(名稱／工作描述／基準級別)＋結構三項(尚無 Duty／有 Task 未歸屬職責／有 Task 未定級別),表頭 issue 排在結構 issue 之前,同一 code 只出一則;零 issue 時安靜;**沒有 `is_complete`／百分比**;說明補充與所屬類別刻意不發聲。**T6 已落地**:`GET /{document_id}` 回 `readiness`,`ReadinessNotice` 直接呈現,零 issue 時不渲染任何東西 |
...
| assembler | Task 現況 → `TaskAnalysisPacket`；單一選定 Task → `OpksContextPacket`；兩者都有決定性 rendering | `application/context.py`、`application/opks_context.py` | 純函式;ordinal 的唯一產地。Task packet 另有 `employee_written_overview`（**只有**職能基準名稱與工作描述，無 ordinal／無 `SourceRef`）。OPKS 只投影選定 Task、有效員工依據、該 Task O/P、全文件 K/S 與相關提案，不送完整 transcript／A／內部 ID；`build_opks_context_packet()` **沒有 header 參數**，工作描述傳不進去 |
...
| PostgreSQL adapter | 0012 四表＋0013 Journal kind＋0014 兩張 OPKS 表＋0015 `jd_header_schema_id`／`jd_header_json`＋**0016 `job_analysis_jd_duties` 與 Task 的 `duty_id`／`competency_level`**＋**0017 OPKS 的 `display_order`**、serialization、repositories、UoW | `app/adapters/job_analysis_postgres/` | OPKS 五種 item 共用一張 typed JSONB 表、獨立 Proposal 共用另一張；不建五表或 refs join table。JSONB 讀取必須 hydrate；schema/shape 壞掉 fail-closed；repository 不 commit。**`jd_tasks.duty_id` 刻意沒有 FK**（比照既有 OPKS `task_refs`）：真 FK 配上 delete-then-insert 的 `replace()` 會讓 `ON DELETE SET NULL` 在每次改 Duty 時默默清空所有 Task 的歸屬——正是本切片要防的資料遺失；參照完整性改由 `JobAnalysisState` 守（指向不存在的 Duty 讀取時整份拒收）。級別的 1–6 由 DB CHECK 直接擋在寫入端。**OPKS 的 `display_order` 是 relational 欄位不進 JSONB**（要排序、要 unique constraint），唯一性範圍 `(document_id, entity_kind)`；0017 的回填**嚴格照 migration 前的 `(created_at, entity_id)` 讀取順序**，既有文件升級後呈現順序逐項不變，有專測守這件事 |
| 匯出組裝 | Current State → `ExportDocument`（位置碼、排序、未歸類集合全算好） | `application/export.py` | 純函式、零 IO、**不 import transport contract**（比照 `assess_readiness()`）。位置碼 `T1`／`T1.1`／`O1.1.1`／`P1.1.1`／`K01` **只存在於回傳值，不落庫**（ADR 0052 決定 10）。未指派 Duty 的 Task 與未連結 Task 的 K/S 各有專屬集合——排不進主表不是讓它從成品上消失的理由 |
| 匯出渲染 | `ExportDocument` → XLSX 位元組 | `application/export_xlsx.py` | 版面依 **2026-08-06 逐份核對的七份官方職能基準範例**：主表**七欄**（最後兩欄是職能內涵 K 與 S，同一個 `K01` 會在多個 Task 列重複出現）、同格內多筆換行、位置碼與文字不留空格、主要職責格垂直合併。**兩個工作表**：公版表（缺漏維持空白，不印提示文字）＋缺漏表（呼叫 `assess_readiness()`，落實 ADR 0052 決定 4）。渲染層不知道 Current State、不算位置碼 |
| authority commit seam | 完整 Current State → 同一 UoW 原子寫入 | `application/authority_commit.py` | 先重驗完整 `JobAnalysisState`（含 JD header 與 OPKS refs），再 replace Duty（**先於 Task**，讓同一交易內看不到指向已刪 Duty 的中間狀態）／JD／Task Proposal／OPKS／OPKS Proposal、寫入 0..N 筆 Journal、generation CAS、單次 commit；edited OPKS 決策用同一 seam 原子寫 proposal-decision 與 direct-edit 兩筆 Journal |
| authoring use cases | 文件庫、JD header、主要職責、JD Task 與 OPKS add/edit/delete/reorder | `application/authoring.py`、`application/duty_authoring.py`、`application/opks_authoring.py` | document row lock → entry replay check → Current State/Journal/generation 同交易；Task 離開 Current JD 時，`prune_opks_for_current_jd()` 同交易移除其 O/P、清理 K/S refs，且不猜接 merge/split 新 Task；不呼叫 LLM。`put_jd_header()` 是最小的一支：header 不影響 Task／Proposal／OPKS，未改內容以 `JdHeaderNotChanged` 拒絕（避免空 Journal 與無關 generation bump），不需要 `_stale_related_proposals()` 或 `prune_opks_for_current_jd()`。Duty 的四支**不動 Work Model**（AI 對職責歸屬沒有權限，標 `pending_reconciliation` 只是雜訊）、**不 prune OPKS**（O/P/K/S 綁 `task_id`，刪職責不刪 Task）；`delete_duty()` 是唯一會改到別的列的一支，它在同一交易把底下 Task 的 `duty_id` 設回 `None`（Task 不跟著消失）**並把那些 Task 仍在等待的提案轉 stale**——後者是正確性不是整潔，見 §5。`reorder_opks_items()` 的範圍是**單一 `entity_kind`**（O 只跟 O 換位置），與 `display_order` 的唯一性範圍一致；新項目取該 kind 的 max+1，**接受提案時才配位置**（提案帶的是佔位值，照抄會讓同回合第二筆被接受時撞號——與 `_apply_jd_entries()` 對 JdTask 同一條理由），replay 比對用**不含位置**的內容指紋 |
...
| Local Web API | 本機文件、JD header、主要職責、Current JD、OPKS 與 Consultation routes | `app/api/routes/job_analysis.py` | 只做 generated wire DTO mapping；不暴露 Work Model、Journal、generation、內部 OPKS Source ID 或 provider detail；Consultation 會投影 Task 與 OPKS 兩種 Proposal，兩種 decision route 各自呼叫對應的 greenfield use case。`PUT …/jd-header` 是最小的一支：要求 `Idempotency-Key`，回應只有 `JdHeaderView`；未改內容以 `JdHeaderNotChanged → 422 invalid-request` 映射，不是 500。Duty 有 `POST`／`PUT`／`DELETE …/duties`＋`PUT …/duty-order` 四條，`DELETE` 回 204、不另開回應形狀（哪幾條 Task 變成未指派，重讀文件就看得到）；Task 的 `duty_id`／`competency_level` **不另開 route**，走既有 `PUT …/tasks/{id}`。`application_error_response()` 對沒映射的錯誤是 `raise TypeError`，所以三個新錯誤（`DutyNotFound → 404`、`InvalidDutyOrder → 422`、`DutyNotChanged → 422`）都必須同時進 problem 映射與 contract 的 `ProblemDetail.type` enum，漏一邊就是 500 |
| Local Web UI | 文件庫、顧問訪談、Proposal 審查、JD 表頭與單一開啟文件的主要職責／Task／OPKS editor | `apps/web/src/app/workspace/`、`components/workspace/` | generated TS DTO + TanStack Query；conversation／Proposal／Current JD 同頁，一份本地編輯草稿、明確儲存，不用 Server Action／autosave／第二份 document store。O/P/K/S 按 Task 投影，同一 K/S 保留同一 entity identity；未連結 K/S 與 A 留在文件層。O/P/K/S/A 各有上下移按鈕，**移動範圍是同一 `entity_kind`**；畫面按 Task 分區但 `display_order` 是文件層 per-kind，所以「相鄰」以看得見的子集合為準、交換發生在完整清單上——這段翻譯是 `lib/jobAnalysisOpks.ts` 的純函式 `kindOrderAfterMove()`（因此有 vitest 覆蓋），有測試守「不會弄丟或重複任何 id」。`JdHeaderForm` 沿用同一條明確儲存慣例並共用 dirty guard；`ReadinessNotice` **只呈現 API 回來的 issues、不自行重算**，零 issue 時不渲染、沒有綠色「完成」，措辭固定為「iCAP 版型欄位尚有 X 項未填」。基準級別是 1–6 下拉且可清空；**沒有 AI 生成按鈕**，也沒有 iCAP 配發代碼的輸入欄。`DutyEditor` 沿用同一條慣例（新增／改名／上下移／刪除，失敗重試沿用同一把 key），刪除前明說「底下的工作任務會保留，但會變成尚未歸入職責」；Task 另有「所屬主要職責」與「職能級別」兩個**可清空**下拉。Current JD 依主要職責分組，分組邏輯是 `lib/jobAnalysisDuties.ts` 的純函式 `groupTasksByDuty()`（因此可被 vitest 覆蓋）：**空職責照樣顯示**（那就是 `duty_without_task` 要讓人看見的缺漏）、**未指派區只在真的有未指派任務時出現**、**任何任務都不會消失**（指向不存在職責的任務歸進未指派區而不是被丟掉——畫面少一條比多一條錯位難發現得多）。**畫面不顯示 `T1`／`T1.1`**，那是匯出版面位置碼不是 identity（ADR 0052 決定 10），有測試守 |

## 3. 一輪的資料流(每步標函式)

1. `build_context_packet(transcript=…, current_turn_id=…, work_model=…, current_jd=…, active_question=…, proposals=…, jd_header=…)`
   → `TaskAnalysisPacket`。順序完全跟隨輸入,所以同輸入同輸出。`jd_header` 只投影成
   `employee_written_overview`(職能基準名稱＋工作描述),其餘表頭欄位不進 packet。
...
| packet 形狀 | 「員工填寫的整體描述」不得成為 task_change 的依據 | 這條**不寫成語意規則**：該區沒有 ordinal，而 anchor 只能指向 packet 裡的 turn ordinal（§12.3），所以模型找不到合法 anchor，逐字 `quote` 檢查會直接擋掉。OPKS packet 連參數都沒有，工作描述無法繞過 0048／0049 的 Evidence 白名單支撐 K/S |
| transition | `jd_header`、`current_duties` 與 Task 的 `duty_id`／`competency_level` 原樣帶過 | AI 對表頭與職責結構都沒有權限：`apply_task_analysis_result()` 與 durable turn 只把它們從既有 state 帶到新 state，永不修改；回歸測試以「拿掉 carry-through 就變紅」證明。**漏帶 `current_duties` 不是靜默清空而是整個 transition 被拒**——剩下的 Task 會指向不存在的 Duty，驗證器當場擋下（T1 就是這樣抓到 `apply_task_analysis_result()` 的真實漏帶）|
| delete_duty | 刪職責不得留下接受不了的提案 | `_apply_jd_entries()` 接受提案時是把 `jd_after` 的 `JdTask` **整份**寫回 Current JD，而那份快照還帶著剛被刪掉的 `duty_id`。因此 `delete_duty()` 必須把受影響 Task 仍 pending／deferred 的提案轉 stale，否則該提案永遠接受不了（state 驗證擋下 dangling `duty_id`），員工只剩「拒絕」一條路。以 mutation check 鎖定：拿掉轉 stale 就變紅 |
...
| 完整 header、匯出 | **header 切片 T1–T7 完成；匯出仍未做**：`JdHeader` 與 readiness 純函式可用，header 是 `JobAnalysisState`／`DocumentRecord` 的一部分並經 0015 兩欄持久化，`put_jd_header()` 已接通 authority seam（entry replay、`JdHeaderNotChanged` 擋未改內容、與 Task／OPKS 同一個 generation CAS），且已有 `PUT /{document_id}/jd-header`／`GET /{document_id}` 兩條 HTTP route 與對應 `job-analysis-contract` wire 型別（`JdHeaderView`／`JdHeaderWrite`／`DocumentReadinessView`／`ReadinessIssueView`）。AI 沒有權限碰它——8 個 state 建構點與 `durable_turn` 的 `update_authority()` 都只把既有 `jd_header` 帶過去，回歸測試以 mutation check 鎖定。**切片已完成**：員工可在 `/workspace` 實際填寫表頭並看到缺漏提示。header 已進 Task Analysis packet 的獨立「員工填寫的整體描述」區（只有職能基準名稱與工作描述）；該區沒有 ordinal，anchor 指不到它，所以「只憑 header 產生 task_change」由 §12.3 的逐字 quote 檢查結構性擋死，不靠語意規則。OPKS packet 不變，`build_opks_context_packet()` 連 header 參數都沒有。**Duty 切片完成（T1–T6）**：`Duty` 與 Task 的 `duty_id`／`competency_level` 已是 domain 形狀、已進 readiness 的三條結構 issue，並經 0016 持久化（真 PostgreSQL 往返、CAS、既有無 Duty 文件照常載入、AI 回合與員工編輯都不動 Duty 均已測）。`add_duty`／`edit_duty`／`delete_duty`／`reorder_duties` 已在 `duty_authoring.py` 落地，走與 Task 同一條 document lock → entry replay → `commit_authority_change()`。wire 型別（`DutyView`／`DutyWrite`／`DutyOrderWrite`、`JdTaskView`／`JdTaskWrite` 的兩個新欄位、`DocumentView.duties` 與 **`ConsultationView.duties`**）、四條 route 與 `/workspace` 的 `DutyEditor`＋兩個下拉＋依職責分組的 Current JD 都已接通——**員工可以在瀏覽器完成整個兩層結構**。**匯出切片完成（ADR 0058）**：`GET /{document_id}/export` 回 XLSX 位元組，`/workspace` 有匯出按鈕；**匯出永遠放行**，缺漏由檔案內的第二張工作表呈現，不阻擋。先做的前置切片是 O/P/K/S/A 的員工可控排序（0017），因為位置碼由排序推出，先出匯出再改排序會讓已交付文件的編號改變。其餘仍只有 Task 與較豐富的內部 JD Task 欄位（`DocumentMetadataWrite` 只有 `title`） | **已由 ADR [0052](../adr/0052-jd-readiness-assessment-and-official-code-boundaries.md) 裁決順序**：先 header／nullable 基準級別／說明補充＋readiness 純函式（**`app/job_analysis` 持有規則**，contract 只承載 view 與 issue codes；只提示不阻止保存／訪談／匯出），再 **Duty＋Task 職能級別的獨立結構切片（export-ready v1 必須完成）**，最後才 deterministic 匯出（不讓 LLM 參與）。**header 走 `JdHeader` 的 authority seam，不是擴充 `DocumentMetadataWrite`；readiness 第一版不回 `is_complete`**（ADR [0053](../adr/0053-jd-header-authority-boundary-and-readiness-scope.md)）。`職能基準代碼`／`職類別代碼` 由 iCAP 配發，不開欄位、不列缺漏、不得生成（0040 決定 33–34、0052 決定 8）；`T1`／`O1.1.1` 是匯出版面位置碼，非 identity（0052 決定 10） |
