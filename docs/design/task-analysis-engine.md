...
| **真模型跑通** | **turn 1 通過**（run `20260731T123344Z`）：`task_analysis_result.v2` 編譯成功並 `committed`。2026-07-31 的三次付費 run 累計 US$0.251,找出三個契約層缺陷,全部已修：grammar 過大、`target_ordinal` 的 1-based／0-based 不一致、6 條 verifier 規則模型無從得知 | **turn 2／3 仍未通過**。turn 2 停在 `RESOLUTION_OPEN_ISSUE_NOT_RECONCILABLE`（已補描述,未重跑）。**未決的設計缺口**：模型自己提的 open issue **沒有任何關閉路徑** —— `resolves_open_issue_ordinal` 依 ADR 0044 只服務 JD-only reconciliation issue,`transition.py` 也只在該路徑 `remove(issue)`；`next_question` 指向它只更新 `last_asked_turn_id`。多輪訪談下 open issue 會單調累積,而 prompt 要求優先處理「仍可取得答案的 open issue」,有重問迴圈的風險。要不要給普通 open issue 關閉路徑是 ADR 級決策,尚未裁決 |
