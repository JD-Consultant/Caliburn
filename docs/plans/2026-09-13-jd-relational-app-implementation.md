...
**唯一下一工作：**接既有 C／B1B2 的真正執行／完整窗口與排空、修補成功後的同輪固定讀取更新及專業指引，集中完成 OI-01／02 的完整顧問流程；不直接 import 研究路徑、不重做 Memory 研究。日常模型維持明示未啟用，自然案例依 OI-09 的資料／呼叫數／預算另驗。其餘依[唯一收尾清單](../specs/2026-09-13-jd-app-open-issues.md)推進；Fetch 只按新證據診斷，不無據重送。當輪改動 UI 真瀏覽器未驗範圍保留，不增舊對話選輪入口。Excel 延後；需求不清或無據增加複雜度時先記錄並討論。
