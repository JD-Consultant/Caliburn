...
**接手先讀（2026-09-13，C 接合完成後）：**[詳細接續計畫](2026-09-13-jd-app-continuation-handoff.md)的 H1–H3 已完成，結果見[App 接合結果稿](../specs/2026-09-13-jd-memory-repair-app-integration-slice.md)：CA-01／02 兩個停止位置已能收尾、真新程序 C 查回與封裝／獨審已收。既有非JD顧問已驗，正在採用到新App；下一從接續計畫 §5 的 H4 開始（採用已驗 B1／B2 與顧問方法），不重做 H1–H3 或原顧問。下列切片「最新／下一」保留當時成果沿革，當前工作以本段及接續計畫為準。本總計畫負責成品範圍，不把局部通過當完整App，不重新廣搜或新增舊聊天選輪功能。
...
**最新 OI-02 讀取接點：**[固定 Memory／原話回查](../specs/2026-09-13-jd-memory-read-integration-slice.md)已接真正模型 request、原生文件工具、詳記來源解析及選版的 native 中斷／收尾；真 SDK／PG 固定三輪與 Windows 宿主回歸通過。0.7 已移除 backend factory，採現行原生工具替換接點；不增加資料表或通用讀取引擎。合成回覆／fixture 發布不等於模型修補或背景整理已完成。

**最新 OI-02 修補核心：**[可安裝 C 子圖與原請求查回](../specs/2026-09-13-jd-memory-repair-core-slice.md)完成官方 patch／原生 staging、typed 來源錯誤與原發布結果核對；真 PG 兩檔更正／回覆遺失／新連線查回與獨審通過。App 模型工具、取消收尾及同輪讀取仍未接，不以核心通過代稱顧問可用。

**唯一下一工作：**將 C 核心接入既有原 call binding／原生 task 觀察、同 owner 取消與未知結果門閘、修補成功後的同輪固定讀取；再接 B1B2 的完整窗口／真正執行／排空及專業指引，集中完成 OI-01／02 的顧問流程。不直接 import 研究路徑、不重做 Memory 研究；以本輪真 PG 已核的固定 checkpoint 取原 request，不推斷 latest overlay 等同保存位置。日常模型維持明示未啟用，自然案例依 OI-09 的資料／呼叫數／預算另驗。其餘依[唯一收尾清單](../specs/2026-09-13-jd-app-open-issues.md)推進；Fetch 只按新證據診斷，不無據重送。當輪改動 UI 真瀏覽器未驗範圍保留，不增舊對話選輪入口。Excel 延後；需求不清或無據增加複雜度時先記錄並討論。
