...
**唯一下一工作：**收尾本次[CV-01 接合](../specs/2026-09-13-jd-run-change-view-slice.md)後，集中完成 OI-01／02 的顧問、Memory／案例、原話來源到 JD 的一條流程；先映射已驗能力的正式接點及專業指引，再以固定 SDK 驗證，不重做已完成的 Memory 研究。日常 AI 未啟用維持可辨識；自然案例按 OI-09 提供資料範圍及預算後另行執行。Fetch OI-03 的有限診斷已達停止條件，只有取得原 Network 階段或可核日常環境證據才續查，不弱化 CORS／保存檢查、不反覆重送。其餘深歷史、選區、撤回、維護及真人驗收按收尾清單的依賴推進；CV-01 真瀏覽器未驗範圍仍保留，不以分層測試代稱完整旅程通過。Excel 延後。需求不清或無據增加複雜度時先記錄並討論。
