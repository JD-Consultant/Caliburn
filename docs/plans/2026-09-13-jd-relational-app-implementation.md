...
**唯一下一工作：**補齊 RS-3 必要接點與故障驗證，優先保存结果遺失／重開、人工更改通知及 AI 回合交接；来源原文、選區、歷史還原／整輪 JD 撤回按既定契約補齊。實體 IME、自然模型與真人驗收分開記錄。文件入口、共用業務與真配置宿主已有，不重造 catalog 或重開同層品牌研究；先以零付費固定輸出接合，不越過模型費用授權。
