...
- **樣式**:新增=綠字+淡綠底;修改=綠字新值+舊值小字副行(沿用 0029 原名副行模式);
  刪除=紅字刪除線。沿用 track-changes 慣例不發明新語彙。
- **粒度=條目級**(一筆 op 一標記;結構化表格天然免疫「整段染綠」噪音)。
- **單筆互動**:hover 浮現 ✓/✗/?;「?」開出處卡=官方來源行+訪談原話(HAX G11)。
- **批量**:工具列「接受全部(N)/拒絕全部」常駐,N=待審數(解標記永不消);分區批量記縫。
- **呈現節奏**:筆級淡入(op 過 verify 才現身),不做表格字元級 streaming;字元級只在側欄
  對話。側欄「正在整理…」狀態指示(介面可靠度=一級需求)。
...
### 7.1 實作偏差記錄(2026-07-13;T1–T14 落地時的降級裁量,皆已註記於碼)

1. **喚醒閘**:鎖定設計寫「consultant 訊號位」(LLM 判斷本回合有無素材);落地改
   **確定性前濾 `worth_scribing`**(meta/寒暄跳過)——LLM 訊號位需動 chat 介面結構化輸出,
   留縫待就緒。寧可多跑一次書記,漏接由 backstop sweep 兜底。
2. **顧問視角**:顧問吃**回合前**文件(自己的落地下回合才看到);綠字由表格即時浮現補位,
   `read_document` 四態視圖供顧問答「你剛記了什麼」。
3. **側欄串流**:B″ 鎖「體感字元級」;落地=前端打字機動畫(零後端改動)。
   真 SSE 端點記縫(觸發:單回合 LLM 延遲 >8s 需提早出字)。
4. **T12 追加退場**:`interview:review` 端點與 web `interviewDoc.ts`(建議套用)隨建議層一併刪
   (plan 附錄成文於 T9/T10 改動前);v1 eval 閘門的 doc_structure 類隨 authoring 退場,
   run_eval 只留 json_zhtw。
