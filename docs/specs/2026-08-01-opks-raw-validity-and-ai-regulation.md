...
2. **強制 task→K/S anchoring。** 這是原著點名的唯一具體對策。Caliburn 的
   「K/S 必須掛 task linkage」正是這條建議的實作；本研究確認它有原著背書，不是自創。
3. **不要用平均分呈現。** 若未來加入重要性評分，要保留 endorsement 計數與加總，
   平均值會**遮蔽**膨脹（p. 682 原文）。
4. **bogus item 是可用的偵測手段。** 產品可在候選清單裡混入**與本職無關的參照 K/S**，
   員工照單全收 → 該次訪談的 K/S 整體降級。這是原著用來測量膨脹的方法，可直接當品質閘。
...
2. **升降級有明文條件**（GRADE 有 −1/−2 的 study limitations、inconsistency、indirectness、imprecision、
   ...
   → Caliburn 的四級目前**沒有降級條件**。依判準 3-B(4)，「員工照單全收 bogus 參照項」應是明文降級條件。
