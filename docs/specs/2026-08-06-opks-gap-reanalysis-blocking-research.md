...
### 3.1 Microsoft Agent Framework — HITL 的 pending request 沒有「全部答完才前進」的閘
...
### 3.3 Ask or Assume?（arXiv 2603.26233v2）——追問本身有大幅價值，但不回答批次問題
...
| 缺口累積問題 | **必須一起解**（§2 末） | **必須一起解** | 不出現 | 不出現 |
...
1. **缺口累積**（§2 末）。傾向：一個 Task 的 active 缺口集合＝**最近一次分析的 uncertain 集合**，
