...
- Provider schema：第一次 exact live run 由 OpenAI strict response schema 拒絕 optional `OutputEvidenceReference.occurrence`。最小修正是 provider wire required strict integer sentinel（唯一 quote 填 `0`、重複 quote 填 1-based），`AnalysisBasisTable` 映射回 workspace 時將 `0` 轉為 `None`，agent prompt 明示規則；沒有新增 abstraction 或放寬 schema。最終 review 再補 boolean／string coercion 拒絕與 ambiguous-quote canary。
