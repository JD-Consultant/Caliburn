...
本次初查後，JD 主線接續處理未歸任務資料、保存後更正、歷史與重開。Owner 後續選定的草稿／還原範圍已由[需求 §11](2026-09-12-jd-relational-editing-requirements.md#11-本輪裁決草稿歷史與還原2026-09-12)及[歷史設計](2026-09-12-jd-history-and-recovery-design.md)承接；工作區最新裁決見下方 §7。LLM workspace 不代替員工草稿需求，也不阻擋已定能力；採用 B／C 前另記取捨與必要設計，ADR0075 Proposed／production0060 不變。
...
## 7. Owner 澄清後的裁決：第一版不設持久 AI 試稿區

Owner 澄清工作區是「因為反覆修改，先在一個區域把 JD 改穩定再提供，類似 LLM code」，並授權研究者評估後繼續。因此本次評估對象是**跨多次操作的 JD 暫存候選**，不是再問是否需要 shell。

定向補核兩份現行官方正文（2026-09-12，產品說明而非新增 OSS 依賴）：[Codex worktrees](https://learn.chatgpt.com/docs/environments/git-worktrees)用 Git checkout 隔離平行修改及驗證，並有交接、清理與恢復生命週期；[Claude Code checkpoints](https://code.claude.com/docs/en/checkpointing)支援受追蹤檔案的回退，但有 shell／其他編輯来源邊界，也不替代長期版本控制。它們證明先修改再檢查、可比較及恢復的價值；不能證明 JD 要使用同一檔案機制或每輪隱藏整份稿。

**研究者受權裁決 WS-01／G3 WORKING：第一版先不設持久 AI 試稿分支。**沿資料足夠再撰寫、相依更正完整提出、App 驗完整候選後整組保存；模型可按需要讀回實際結果並再次更正。目前稿會隨每次 confirmed 業務操作更新，員工能看到真實變動；不等待整份 JD 被宣稱完美才第一次提供。

這個選擇不將 App 的結構驗證誇大為專業品質驗證，也不把單次交易候選說成模型已跨工具反覆試稿。同一更正不能因工具容量不足被拆成互相矛盾的多次發布；遇到既有完整操作無法表達的具體反例，先重開該操作或候選能力，不能以「沒有工作區」合理化半成品。AI 回合可能有多個獨立已保存效果，取消只停止後續工作；已保存者按新[歷史與恢復設計](2026-09-12-jd-history-and-recovery-design.md)處理。

重開條件：出現「須跨多次編修比較、未採用稿要中斷續作、檢查完成前目前 JD 必須維持不變」的明確用途，或自然驗收證實既有完整操作造成無法接受的反覆發布／品質缺口。屆時先沿本稿 B 的有限 JD 候選生命週期比較，不直接加完整檔案 sandbox。未完成操作筆記仍是另一問題。此裁決不採新工具／資料表，也不變更既有 Memory staging。
