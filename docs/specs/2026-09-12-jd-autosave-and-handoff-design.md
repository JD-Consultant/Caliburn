...
**歷史、切文件及封存：**同頁歷史可唯讀展開。正常切文件或封存先等待已送結果並處理未提交候選；有未解結果時提供留在原頁／查看狀態／恢復的出口。原 operation 與全部待保留輸入已確認持久暫存時，可明示保留後切其他文件，原文件 gate 不解除，詳[重開出口](2026-09-12-jd-history-and-recovery-design.md#6-關頁切文件與重開的具體出口)。封存仍經 server gate，不能跳過 pending writer。正常關頁以尚未保存提醒輔助，不能依賴 beforeunload 完成最後保存。重開依 §5 恢復。
