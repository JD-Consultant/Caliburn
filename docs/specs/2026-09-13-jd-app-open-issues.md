...
### OI-05｜對話／來源／Memory 的長訪談連續性：接線後核實
- 現象／影響：canonical conversation lineage 查找有 256 祖先上限及缺鏈出口，底層讀完整 messages；長上下文、壓縮與晚期更正尚未在新 App 完整驗證，不能把上限或缺鏈當「沒有原回合」。這是 LLM 工作理解與原話查回問題，不是 JD 版本歷史功能。
...
- 下一最小動作：正式接線後核對 conversation／source／Memory context 交接，保留原話與原 request 身分；先用一個超過現有查找窗口的合成流程驗證，不為此增加 JD 歷史 UI 或通用版本引擎。
...
- 下一最小動作：接線 E2E 只核對既有整輪撤回仍走共用 JD domain writer、能顯示該輪差異、遇較晚修改會安全拒絕；不新增通用 undo stack、歷史選輪或任意早期局部拒絕引擎。
- 退出條件：撤回形成新的安全 JD 寫入，原對話、來源、Memory、案例、工作理解、checkpoint 與原回合紀錄保留；有較晚修改時不覆蓋，結果遺失查回原 operation 不重複套用。
...
### OI-09｜接線後自然模型 smoke：顧問品質與 prompt 基線已完成
- 現象／影響：Luna 顧問的自然模型試用與 prompt 校準已完成，不重做模型選擇或廣泛品質批次；新 App 尚缺的是接線完成後，以同一 JD writer／context 跑通的窄自然旅程。合成 SDK／真 DB 只能證明工程接點，不能代替最後一次真 route 核對。
...
- 下一最小動作：OI-01／02 接好後，只執行能證明 OpenRouter／Luna 正式入口、context、工具保存回執與錯誤出口的一個最小自然 smoke；先列資料範圍、呼叫數與費用上限，沿有效授權執行。
- 退出條件：真 route 為 OpenRouter 的 OpenAI-only `openai/gpt-5.6-luna`、無 fallback；人編輯與 LLM 編輯落到同一 current JD，當輪差異可見，原對話／Memory 保留。更廣的多職位品質回歸可後續安排，不阻止先完成可用接線。
...
OI-04 工程接合已完成，接著集中 OI-01／02 的顧問、Memory 與來源流程；OI-04 未驗操作隨該完整旅程集中驗收。OI-03 只按新增證據定點處理；OI-05 隨 context／Memory 接線核實，OI-06 只守住既有撤回 invariant，不擴張歷史／還原；OI-07／08 再依相依收尾。OI-09 只做接線後最小自然 smoke，OI-10 後續安排；未達到的必要退出條件如實標未完，不以「大致完成」改成成品 PASS。
