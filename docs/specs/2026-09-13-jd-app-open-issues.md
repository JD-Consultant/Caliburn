...
- 下一最小動作：依映射§6第5項**把整理通知註冊給顧問並採用A指引／三項分析Skills**——需同時處理工具註冊、結果分類、關閉與停止路徑識別此純通知（不借JD operation或C publication），以及既有`expected_tool_count`斷言；這是一個完整工作單位，不拆半步。之後才進B1／B2採用，並在該處補F-02／F-04／F-05。現有C／只讀工具的source port不動。
