...
- 現象／影響：新 App 尚未接完整 Memory／案例能力及實際 source owner；來源 metadata／`not_checked` 不代表能回查原話。這直接影響反覆更正、忠實度與客製化，不能當美化缺口。
...
- 下一最小動作：按既定資料責任接目前理解／案例／原話讀寫，核最新尚未整併的對話如何進入本輪判斷及 JD 依據；保留各自唯一權威，不重包舊產品或另存一份理解。
