...
- **2026-09-14 進度（二）：**預覽流程、HTTP／Web 入口與整輪 AI 撤回都已接線，並完成[真瀏覽器驗收](evidence/2026-09-14-jd-restore-and-undo-browser-results.md)：真 Chrome 上建立→打字→自動保存→開歷史→「還原到第 2 版」→看逐項比較→「確認還原」，以及一次真訪談讓 AI 寫入後按「撤回這輪 JD 改動」→「確認撤回」；兩條都由獨立唯讀連線核對 PostgreSQL——還原／撤回各新增一版、被取代的版本仍讀得到、conversation checkpoint 未減少。**本項仍未關閉**：還原確認期間尚未暫停手改與新 AI 回合（設計 §4.2，目前只有保存時 `stale_view` 拒絕），收據也只記 `command_kind`、未記被撤回的回合與實際起點。
