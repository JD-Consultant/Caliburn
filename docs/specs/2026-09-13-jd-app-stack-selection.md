...
**Owner 續行澄清（2026-09-13）：**本稿的框架方向均為可替換的工程選擇，不是固定產品需求或 Owner 技術偏好。若候選不能承接已定管理效果，替換候選，不為框架改成文章編輯器；僅因有更新品牌／版本不重開廣搜。先執行[完整任務與一次更正切片](2026-09-13-jd-relational-command-slice.md)，RS-F 依相依部分閉合，不讓畫面選型阻止無相依的共同業務驗證。
