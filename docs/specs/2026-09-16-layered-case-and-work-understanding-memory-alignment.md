...
5. **案例引用精確化（已完成）＋完整背景 job／共同 publication（下一片）：**B1 已從「整批來源自動附到每個改動案例」改為 Runtime 驗證的完整回合引用選擇與 split／merge 分配；bundle 也會由固定歷史上界重新證明 lineage 與 owner order。下一片只串 B1 staged → B2 staged → candidate bundle → 一次 publication，並消費 `case_rework_required`，最多自動退回 B1 一次；驗證 B1 成功而 B2 失敗不外露、semantic no-op、checkpoint resume 及 stale 後 B1／B2 重整。
