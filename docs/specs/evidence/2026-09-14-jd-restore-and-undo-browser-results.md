...
| 按「看第 1 段原話」 | 面板內**員工原話一字不差**並標為「你說的」。這一段來源只涵蓋該輪員工自己的輸入，所以面板裡沒有顧問的話，也**沒有**去附註一段不存在的顧問回覆——註解只說「這段訪談本身不會因為 JD 改動而變」。判讀只讀該面板，不讀整頁：訪談欄裡也有同一段話，用整頁比對什麼都證明不了。 |
...
- 這條旅程只有**新增**與**加入引用**兩種改動。修改／刪除／移動的辨認仍未在真瀏覽器驗；依 [OI-04](../2026-09-13-jd-app-open-issues.md) 的既定做法，那要**配合核心顧問旅程**核對，不以擴充固定離線腳本代替——該腳本的定位是接線驗收，不是訪談模擬。
