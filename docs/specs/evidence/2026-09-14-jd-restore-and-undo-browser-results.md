...
| 實際操作 | 觀察 |
|---|---|
| 建立「整輪撤回驗收」，在訪談框輸入一段自己的工作敘述，按「送出」 | 一次真回合：AI 自己呼叫 `jd_read`、`jd_create_task` 寫入，畫面出現「已保存 1 次 JD 修改」與 1 個任務、2 項成果、1 項要求。 |
| 看「本輪 JD 改動」 | 逐項列出這輪的新增（任務／要求／成果）與變動欄位，並說明「查看不會更動 JD」。 |
