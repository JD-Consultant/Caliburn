...
- 日期：2026-09-10；狀態：候選，供一次窄 review；尚未採用／實作／實測。
...
| `unknown` | `request_key`、`can_recover: bool` | 有匹配的可信 pending 身分，尚無 terminal receipt。can_recover 是目前可否嘗試明示恢復的 snapshot，不是停止證明或預先 admission。 |
...
1. 同文件 admission lock 內 canonicalize key，計算既有 operation，先讀該 operation terminal receipt。若存在，回原 available；可對仍匹配的 root descriptor 做既有終局 clear。clear 失敗不改寫真結果，留下待清狀態並維持所需 gate；若無法確認 gate，不授予新 admission。
...
- available 證明僅限該 operation 的 terminal。它不證明兄弟 read、其他 writer、AI bindings／ToolMessage／manifest 已閉合；解除寫 gate 仍由 Task 5 同一完整 owner admission 判斷。不能因 no_pending 或 available 在 client 單獨解所有鎖。
...
掛載／pageshow 先做唯讀 manual discovery，與現有 metadata/run/head 讀取協作；若有本地 exact candidate，用其原 request_key 查詢，並且仍做一次無 key discovery 以辨識是否存在別的 current pending。這是有限兩次 GET，不是輪詢或歷史枚舉。無 candidate 只需無 key discovery。

unknown 必須有不依賴 candidate 的區塊，例如「上次人工保存結果尚未確認」，按鈕「確認上次保存結果」明示 POST 該 key；can_recover=false 時顯示仍在處理，容許唯讀重新整理。沒有 cache 時說明無法還原未保存候選，但可以確認原保存結果；不能把目前 JD 填成原候選。每次人工點擊只派一次 POST，fetch 不自動 retry。API 錯誤保留狀態；unknown 結束 spinner 保留入口。
...
- no_pending：表示所查身分無 pending/receipt；不能清 candidate、標零寫入或自動重送。若有未明 cache 且無可信 descriptor/receipt，是明確診斷缺口（例如 admission 前 HTTP 失敗或狀態遺失），保留候選與原未知語意，不能從 absence 假裝 known-none。Task 4 明確 not_admitted 的 affirmative marker 仍按原流程處理，不能把此新分支當同一 marker。
...
load 的自動 `save(true)` 不再用於未知狀態。終局 receipt 可由 GET 查回；已明確 not_admitted／可重新送出之候選仍需沿既有明示 exact submission 操作，不讓恢復 POST承擔保存。若舊 cache 沒記住明確 not_admitted，不能用新 GET absence 代替該證據；這個限制需如實顯示，不新增持久候選 store 解決。

## 5. 契約與 implementation 接合範圍

- 在 actual `jd_routes.py` Pydantic 定 request／discriminated response DTO，回 available 的 result 引用既有 generated `JdManualSaveResult` 型別；只在 mapper adapter 中轉換內部結果。domain 不 import 新 transport DTO。
...
| MT06 | terminal receipt 有值但 descriptor clear 失敗或其他 owner outstanding | 可讀真 available；不得假定全文件可寫。後續 clear 只影響匹配身分。 |
...
| MT12 | actual Pydantic schema 與 Web build／生成check | 新 union 可判別、UUID/extra校驗一致、無手寫平行response；三工具 request shape 完全不變。 |
...
1. journey §6.2 原 manual「不新增 route／結果枚舉」改為：manual-save exact submission 與模型工具契約不變；為已採 cache-lost 效果新增 App-only GET/POST manual-recovery，唯讀原 key 發現／原 receipt，以及另次明示一次對帳，不重跑 candidate。§7 加原 pending 恢復與 receipt 都先於 archive 阻擋新意圖；§9/J05–J08 引 MT race。
2. lifecycle §4.2 補明示恢復的 HTTP 接點與 active attempt 不可重入；§6.4–6.5 補 browser 只傳原 request_key、server 取可信 descriptor digest、terminal 後同 key 可查原 receipt；§7 Task 5 Files 加 actual DTO export、generated Web及api wrapper；不改停止／PG proof。
3. core plan Task 5／brief §5.1、5.4、5.5 加 MT01–12 與上述 Files；Task 4 已接受結果保持歷史原貌，只說 load unknown 行為由 Task 5 接替。
4. Proposed ADR0074 Decision 7 lifecycle/manual identity 段補「App-only 原key唯讀發現及一次明示恢復；无candidate重播或新authority」，Consequence 補必要兩方法與生成 DTO seam；維持 Proposed／隔離採用／production gate不變。
