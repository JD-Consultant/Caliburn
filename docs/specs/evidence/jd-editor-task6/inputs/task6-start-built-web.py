...
for key in ('Q019_TEST_DATABASE_URL', 'OPENAI_API_KEY', 'OPENROUTER_API_KEY', 'ANTHROPIC_API_KEY', 'LANGSMITH_API_KEY'):
    env.pop(key, None)
with (ROOT/'scratch/task6-built-web.log').open('w', encoding='utf8') as out:
...
(ROOT/'scratch/task6-built-web-server.json').write_text(json.dumps(result, indent=2), encoding='utf8')
