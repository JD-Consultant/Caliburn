...
await run('F01-A', '完整 r2 容器 normalization、JSON 存檔及新 editor 重開', async (e, trace) => {
  ...
  assert.deepEqual(reopened.children, fixture);
  assert.deepEqual(unwrappedOracle(reopened.children), source);
...
await run('F01-D', '未完整與未歸屬內容可保存；空容器另作觀測', async (e, trace) => {
  ...
  e.tf.insertNodes(additions, { at: [...workPath, node(e, 'section-work').children.length] });
