...
const oldDuty=work.children.find(n=>n.id==='duty-4');
const originalIndex=oldDuty.children.findIndex(n=>n.id==='task-8');
if(originalIndex<0)throw Error('Fixed fixture Task8 no longer directly under duty-4');
const task8=oldDuty.children.splice(originalIndex,1)[0],duty=work.children.find(n=>n.id==='duty-1');
...
write('fixture-statistics.json',{
  ...
  task8:{from:{parent:'duty-4',index:originalIndex},to:{parent:'duty-1',index:destinationIndex},subtree:'task8-subtree-expected.json',fullExpected:'task8-move-full-expected.json',
