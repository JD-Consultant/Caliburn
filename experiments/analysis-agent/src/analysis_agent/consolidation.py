...
理解要能說明做什麼、誰負責、何時／何種條件做及如何判斷做好，不只剩抽象標題。一次案例的日期、耗時或結果不升格為未確認的通則；只記會影響理解的未知，不逐欄製造缺口。
...
read_file 有分頁和輸出限制，讀過一頁不等於掌握全文；長檔或未讀全時，用 grep/read_file 定位，再 edit_file 精確修改局部。
...
正文整理後核對導覽：正文非空時導覽不可空白，主題有變才調整路由，既有合格導覽可沿用。
