...
@pytest.fixture
def publications():
    assert importlib.util.find_spec("analysis_agent.publication"), "Missing atomic publication seam"
...
def test_stale_background_cannot_overwrite_repair_or_advance_cursor(publications):
    from analysis_agent.publication import StalePublication
    pub, artifacts = publications
    _, _, source, _ = setup_source()
...
def test_lost_commit_response_can_be_reconciled_without_republishing(publications):
    ...
    from analysis_agent.publication import PublicationStore, PublicationUncertain
    pub, artifacts = publications
    req = request(pub, artifacts)
...
def test_reconciliation_connection_failure_preserves_uncertain_contract(publications, failure_point):
    from sqlalchemy import event
    from sqlalchemy.exc import OperationalError
    from analysis_agent.publication import PublicationUncertain, StalePublication
    pub, artifacts = publications
