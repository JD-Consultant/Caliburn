...
**目前沒有可開啟的 App 畫面；八操作已透過 Python service 保存到獨立 PostgreSQL 的十三表。**已接六章／歷史與原次改動的兩個 HTTP 查詢，通過真 PG 及真 Uvicorn 新程序。`build_candidate` 仍只回保存前候選，`JdStorage` 在完整交易確認後才回已保存結果。實際 writer／持久 runtime、寫入 HTTP、畫面及顧問接線尚未完成。此目錄不讀寫 Memory／訪談、不呼叫模型、不接正式產品或舊實驗模組。
...
`WriterAuthority` 只有明示測試替身，不能讓 HTTP caller 自稱有寫入資格或程序已停止。`JdReader` 無需該替身；兩個查詢 HTTP 只使用 `JdReader`／`HistoryReader`。簽章 key／dataset 仍由宿主注入；同 key 新程序不代表已完成宿主持久設定或備份還原世代。`jd_read`／`jd_change_read` 契約、分頁及兩家工具外殼已驗。寫入 HTTP、畫面、人工變更通知及顧問 runtime 尚待接線；7 欄操作身分的新程序恢復不代表真停止 owner 已完成。
...
歷史[結果與資料庫基礎](../../docs/specs/2026-09-13-jd-result-and-storage-foundation.md)、[共同保存交易](../../docs/specs/2026-09-13-jd-transaction-service-slice.md)及[同版讀取](../../docs/specs/2026-09-13-jd-read-change-implementation.md)保留當時結果。最新測試、首敗、独立審查及下一工作見[查詢 API 與恢復身分](../../docs/specs/2026-09-13-jd-query-api-and-recovery-identity-slice.md)；各批數字不累加。
