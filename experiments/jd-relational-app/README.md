...
**目前狀態（2026-09-17）：**A／B1／B2 的正式角色模型現由單一 App factory 透過同一 OpenRouter credential 與 process-owned shared clients 建立，固定 OpenAI-only `openai/gpt-5.6-luna`、禁止 fallback、零 hidden retry；建構不送出模型請求。A 仍使用既有顧問 graph，B1／B2 model objects 已由 process runtime 保留，等待下一片 document-scoped managed callback 注入已完成的 `BackgroundMemoryWorkflow`。App-side continuity compaction、分層 Memory workflow、dispatcher 資源與恢復切片均已完成各自離線驗證；仍未完成 managed callback、layered C、自然模型／付費驗證、完整 App journey 與 production authority。正式 B1／B2 model-step、tool-call 及來源窗口上限尚未由 fixture 數值升格為產品政策。精確邊界見[角色模型工廠](../../docs/specs/2026-09-17-openrouter-role-model-factory.md)與[目前決策](../../docs/current-decisions.md)。
...
- `openrouter_model.py`／`consultant_model.py`／`role_models.py`／`consultant_runtime.py`：App 共用 OpenRouter transport／route／terminal evidence、既有 Luna profile 與 process-owned clients；正式 factory 以單一 credential 建立 A／B1／B2，全部 OpenAI-only、禁止 fallback、零 hidden retry。A 使用原顧問 graph；B1／B2 等待 managed callback 注入既有 workflow。舊 direct Responses／native compaction 只保留歷史契約證據，不接回現行路徑。
