...
A is pinned to Anthropic; B1 and B2 are pinned to OpenAI. There is no app-wide
provider switch and no fallback between them. Nothing here opens a connection,
creates a document, starts background work or grants writer authority.
"""
...
def build_consultant(model, *, admissions=None, publication=None, windows=None,
                     document_id=None, tools=None, guidance=None):
    """One document's consultant, with its method, its Skills and its notices.

    Background availability is included only when the callers that own those
    durable readers supply them; a host without background wiring still gets a
    consultant that can interview, read Memory and write JD.
    """
    background = (admissions, publication, windows, document_id)
    ...
    if all(value is not None for value in background):
        middleware.append(BackgroundAvailability(admissions, publication, windows, document_id))
