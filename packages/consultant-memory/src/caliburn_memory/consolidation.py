...
Adopted from the verified analysis agent: the prompt, the staged-file loop,
the model/tool budgets, stale->load, RECENT_REPAIRS and the artifact-derived
operation identity are unchanged. The one adaptation is that this package
binds no provider, so the context view middleware a provider needs is passed
in by the App rather than imported here.
"""
...
from .staging import consolidation_tools, staged_texts
from .consolidation_feedback import ConsolidationFeedback, completed_messages
from .memory import MemoryVersion
from .publication import PublishRequest, StalePublication
...
class ConsolidationWorkflow:
    def __init__(self, extraction, publication, model, checkpointer, *,
        ...
        # How a provider's context is viewed (inline compaction, for instance)
        # belongs to whoever assembled that provider, so the App supplies it.
        # This package binds no client and knows no provider.
        self.context_middleware = tuple(context_middleware)
    ...
    def _consolidate(self, state: JobState) -> dict:
        ...
        agent = create_agent(model=self.model.model_copy(update={"max_tokens": self.max_output_tokens}),
            ...
            middleware=[ConsolidationFeedback(self.artifacts), *self.context_middleware,
